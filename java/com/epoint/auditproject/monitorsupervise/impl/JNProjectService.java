...
public class JNProjectService {
    ...
    public PageData<auditprojectold> getAuditProjectPageDataMonitorOld(String fieldstr, Map<String, String> conditionMap, Integer firstResult, Integer maxResults, String sortField, String sortOrder) {
        PageData<auditprojectold> pageData = new PageData<>();
        SQLManageUtil sqlManageUtil = new SQLManageUtil("project", true);
        String sql = "select " + fieldstr + " from audit_project_old";
        String sqlCount = "select count(1) from audit_project_old";
        String sqlCondition = sqlManageUtil.buildSql(conditionMap);
        String sqlOrder = "";
        if (StringUtil.isNotBlank(sortField)) {
            sqlOrder = " order by " + sortField + " " + sortOrder;
        ...
        List<auditprojectold> dataList = commonDao.findList(sql + sqlCondition + sqlOrder, firstResult,
                maxResults, auditprojectold.class);
    ...
    public auditprojectold getAuditProjectByRowGuid(String projectguid) {
        ...
        sql += " from audit_project_old where rowguid = ?";
        return commonDao.find(sql, auditprojectold.class, projectguid);
    }
