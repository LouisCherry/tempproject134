 ...
 */
@RestController("jndmonitorsuperviselistaction")
@Scope("request")
public class JNDMonitorSuperviseListAction extends BaseController {
     ...
     * 表格控件model
     */
    private DataGridModel<auditprojectold> model;
    /**
     * 导出模型
     */
    ...
    public DataGridModel<auditprojectold> getDataGridData() {
        // 获得表格对象
        if (model == null) {
            model = new DataGridModel<auditprojectold>() {

                @SuppressWarnings({"unchecked", "rawtypes"})
                @Override
                public List<auditprojectold> fetchData(int first, int pageSize, String sortField, String sortOrder) {
...
//                            sql.isBlank("is_lczj");
                        ...
                        PageData<auditprojectold> pageData = jnProjectMonitorService.getAuditProjectPageDataMonitorOld(
                                fieldstr, sql.getMap(), first, pageSize, sortField, sortOrder).getResult();
                        List<auditprojectold> pagelist = new ArrayList<auditprojectold>();
                        int pagecount = 0;
                        if (pageData != null) {
