 ...
 */
@RestController("jnmonitorsuperviselistaction")
@Scope("request")
public class JNMonitorSuperviseListAction extends BaseController
{
            ...
            model = new DataGridModel<AuditProject>()
            {
                ...
                public List<AuditProject> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if (StringUtil.isNotBlank(areaCode)) {
                        ...
                        // 接件时间
                        if (StringUtil.isNotBlank(receiveDateStart)) {
                            sql.ge("RECEIVEDATE", EpointDateUtil.getBeginOfDateStr(receiveDateStart));
                        }
                        if (StringUtil.isNotBlank(receiveDateEnd)) {
                            sql.lt("RECEIVEDATE", EpointDateUtil.getEndOfDateStr(receiveDateEnd));
                        }
                        if (is_leader) {
                        ...
                        sql.setSelectFields(
                                "chaoxiandetailurl,rowguid,status,pviguid,projectname,windowname,flowsn,applyername,applydate,ACCEPTUSERDATE,ouname,RECEIVEDATE,RECEIVEUSERNAME,CERTNUM,contactphone,ACCEPTUSERNAME,BANJIEUSERNAME");
                        PageData<AuditProject> pageData = auditProjectService
                                .getAuditProjectPageDataMonitor(sql.getMap(), ismaterial, first, pageSize, "", "")
                                .getResult();
                        ...
                        if (pageData != null) {
                            ...
                            if (chaoshi) {
                                for (AuditProject auditProject : pagelist) {
                                    ...
                                    if (!getSpareTime(auditProject.getRowguid(), auditProject.getStatus())
                                            .startsWith("超时")) {
    ...
    public String getSpareTime(String rowguid, int status) {
        ...
        if (StringUtil.isNotBlank(rowguid) && StringUtil.isNotBlank(status)) {
            ...
            AuditProject auditProject = auditProjectService.getAuditProjectByRowGuid(fields, rowguid, "").getResult();
        ...
        if (statusModel == null) {
            ...
            if (statusModel != null) {
                ...
                while (it.hasNext()) {
                    ...
                    // 删除列表中不需要的列表选项
                    if ((Integer.parseInt(item.getValue().toString()) == ZwfwConstant.BANJIAN_STATUS_WWINIT)
    ...
    public void deleteSelect(String ids) {
        if (StringUtil.isNotBlank(ids)) {
            ...
            for (String projectGuid : select) {
                ...
                // 如果存在待办事宜，则删除待办
                List<MessagesCenter> messagesCenterList = messagesCenterService.queryForList(userSession.getUserGuid(),
                        null, null, "", IMessagesCenterService.MESSAGETYPE_WAIT, projectGuid, "", -1, "", null, null, 0,
                        -1);
    ...
    public ExportModel getExportModel() {
        if (exportModel == null) {
            exportModel = new ExportModel(
                    "windowname,ouname,projectname,flowsn,applyername,CERTNUM,contactphone,applydate,RECEIVEUSERNAME,RECEIVEDATE,ACCEPTUSERNAME,ACCEPTUSERDATE,sparetime,BANJIEUSERNAME,banjiedate,status",
                    "窗口名称,办件所属部门,事项名称,办件编号,申请人,证件编号,联系方式,申请时间,接件人,接件时间,受理人,受理时间,办结剩余时间,办结人员,办结时间,办件状态");
        }
        return exportModel;
    }
     ...
     * @param projectguid
     */
    public void replay(String projectguid){
        AuditProject projectDetail = auditProjectService
                .getAuditProjectByRowGuid(projectguid, ZwfwUserSession.getInstance().getAreaCode()).getResult();
        AuditTask task = iAuditTask.getAuditTaskByGuid(projectDetail.getTaskguid(),true).getResult();
        WavePushInterfaceUtils wavePushInterfaceUtils = new WavePushInterfaceUtils();
        wavePushInterfaceUtils.TsLcGjk(projectDetail,task,userSession);
        addCallbackParam("msg", "补推成功！");
    }
