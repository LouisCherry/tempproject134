...
@RestController("jnwindowallauditbanjianlistaction")
@Scope("request")
public class JNWindowAllAuditBanJianListAction extends BaseController
{
        ...
        if (model == null) {
            model = new DataGridModel<AuditProject>()
            {
                ...
                public List<AuditProject> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                  ...
                  //  sql.eq("windowguid", ZwfwUserSession.getInstance().getWindowGuid());
                    ...
                    if (StringUtil.isNotBlank(dataBean.getBanjieresult())) {
                        sql.eq("banjieresult", dataBean.getBanjieresult() == null ? "0": dataBean.getBanjieresult().toString());
                    }
//                    sql.eq("status", String.valueOf(ZwfwConstant.BANJIAN_STATUS_ZCBJ));
                    sql.in("status", "97,90");
                    String taskids = "";
                    ...
                    if (taskidList != null && taskidList.size() > 0) {
                     ...
                     String fields = "rowguid,taskguid,projectname,applyeruserguid,applyername,areacode,pviguid,status,tasktype,flowsn,applydate,banjieresult,banjiedate,spendtime,contactmobile,contactcertnum,certnum ";
