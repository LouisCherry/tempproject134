 ...
 */
@RestController("jnauditprojectinfosgxkaction")
@Scope("request")
public class JNAuditProjectInfo_SGXKAction extends BaseController
{
    /**
     * 
     */
    ...
    private List<SelectItem> gongChengYongTuModel =null;
    
    private List<SelectItem> zhongdianxiangmuModel =null;
    
    private List<SelectItem> InvPropertyNumModel =null;
    /**
     * 是否测试办件Model
     */
...
log.info("update promise_day-task:"+audittask.getItem_id()+","+audittask.getPromise_day()+",user:"+userSession.getUserGuid());
            ...
            if (auditproject != null) {
               ...
               /* @SuppressWarnings("serial")
                ...
                if (!isPostback()) {
                    ...
                    //system.out.println("dataid:"+auditProject.getStr("dataid"));
                    //调用浪潮接口获取到对应的施工许可证信息
                    if (StringUtil.isNotBlank(auditProject.getStr("dataid")) && auditProjectFormSgxkz == null) {
                        auditProjectFormSgxkz = new AuditProjectFormSgxkz();
                        ...
                        String result = HttpUtil.doGet("http://59.206.96.198:8030/cform/getFormStandardData?formId=ShiGongXuKeZhengNEW&formDataId="+auditProject.getStr("dataid"));
                        //system.out.println("result:"+result);
                        Document document = DocumentHelper.createDocument();
                        Element root =  document.addElement("stakeapply"); //默认根节点
                        try {
                            Element el = jsonToXml(result, root);
                            parseXmltoEntity(el.asXML(),projectguid,auditProject.getStr("dataid"));
                            auditProject.set("form_xml", el.asXML());
                    ...
                    //system.out.println("auditProjectFormSgxkz:"+auditProjectFormSgxkz);
                    /*if(auditProjectFormSgxkz == null){
                        ...
                        if(StringUtil.isNotBlank(form_xml)){
                            parseXmltoEntity(form_xml,projectguid,auditProject.getStr("dataid"));
                        }
                    }*/
                }
    ...
    public void parseXmltoEntity(String form_xml,String projectguid, String dataid){
        Document document;
        try {
            ...
            for(Element attr:listAttr){//遍历当前节点的所有属性
                String sgxkzname=attr.getName();//属性名称
                String sgxkzvalue = attr.getText();
                if("XiangMuMingONE".equals(sgxkzname)) {
                    auditProjectFormSgxkz.set("XiangMuMingChen", sgxkzvalue);
                }else if("XiangMuBianHaoONE".equals(sgxkzname)){
                    auditProjectFormSgxkz.set("XiangMuBianHao", sgxkzvalue);
                }else {
                    auditProjectFormSgxkz.set(sgxkzname, sgxkzvalue);
                }
                //system.out.println("项目属性名称:"+sgxkzname+"项目属性值:"+sgxkzvalue);
            }
            iAuditProjectFormSgxkzService.insert(auditProjectFormSgxkz);
            List<Element> dantis = rootElement.elements("DanTiJiBenShiGongONE");
            for (Element idanti : dantis) {
                AuditProjectFormSgxkzDanti danti = new AuditProjectFormSgxkzDanti();
                danti.setOperatedate(new Date());
                danti.setFormid(dataid);
                danti.setFormguid(auditProjectFormSgxkz.getRowguid());
                danti.setRowguid(UUID.randomUUID().toString());
                danti.setProjectguid(projectguid);
                for(Iterator<Element> it = idanti.elementIterator();it.hasNext();){
                    Element e = it.next();
                    String name2=e .getName();//属性名称
                    String value2=e .getText();//属性的值
                    if("DanTiJianGouZhuWuNEW".equals(name2)) {
                        danti.set("DanTiJianGouZhuWuMingChen", value2);
                    }else {
                        danti.set(name2, value2);
                    }
                    //system.out.println("单体属性名称："+name2+"属性值："+value2);
                    }
...
//            Collections.reverse(certtypeModel);
    ...
    @SuppressWarnings("unchecked")
    public List<SelectItem> getInvPropertyNumModel() {
        if (InvPropertyNumModel == null) {
        	InvPropertyNumModel = DataUtil.convertMap2ComboBox(
                    (List<Map<String, String>>) CodeModalFactory.factory("下拉列表", "工程投资性质", null, false));
        }
        return this.InvPropertyNumModel;
    }
    @SuppressWarnings("unchecked")
    ...
    @SuppressWarnings("unchecked")
    public List<SelectItem> getGongChengYongTuModel() {
        if (gongChengYongTuModel == null) {
            gongChengYongTuModel = DataUtil.convertMap2ComboBox(
                    (List<Map<String, String>>) CodeModalFactory.factory("下拉列表", "工程用途", null, false));
        }
        return this.gongChengYongTuModel;
    }
    
    @SuppressWarnings("unchecked")
    public List<SelectItem> getZhongdianxiangmuModel() {
        if (zhongdianxiangmuModel == null) {
            zhongdianxiangmuModel = DataUtil.convertMap2ComboBox(
                    (List<Map<String, String>>) CodeModalFactory.factory("下拉列表", "项目等级", null, false));
        }
        return this.zhongdianxiangmuModel;
    }
    ...
    public void getgongandetail(String certnum,String name) throws IOException, DocumentException{
		...
		if(responseJsonObj!=null && responseJsonObj.getString("code").equals("200")){
			////system.out.println("请求数据成功 ，返回值为："+responseJsonObj.getString("data"));
  ...
  public static Element jsonToXml(String json, Element parentElement) throws Exception {
      ...
      String rows = js.getString("rows");
      JsonObject jsonObject = new JsonParser().parse(rows).getAsJsonObject();
