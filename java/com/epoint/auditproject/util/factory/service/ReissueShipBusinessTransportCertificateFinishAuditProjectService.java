 ...
 * @Description《船舶营业运输证》补发
 * @Author rachel
 * @Date 2023/5/26 17:28
 **/

@Component
public class ReissueShipBusinessTransportCertificateFinishAuditProjectService extends AbstractFinishAuditProjectService {

    @Override
    public void handleBusiness(AuditProject auditProject, AuditTask auditTask) {
        // 《船舶营业运输证》补发
        try {
            log.info(">>>>  《船舶营业运输证》补发开始！");

            if (auditProject == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，办件为空，请检查逻辑！");
                return;
            }

            log.info("船舶营业运输证》补发，办件办结后事件，当前办件唯一标识：" + auditProject.getRowguid() + "！");

            if (auditTask == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，事项为空，请检查逻辑！");
                return;
            }
            ...
            if (auditTaskExtension == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，事项拓展信息为空，请检查逻辑！");
                return;
            }

            String formId = auditTaskExtension.getStr("formid");
            if (StringUtil.isBlank(formId)) {
                log.info("《船舶营业运输证》补发，办件办结后事件，事项拓展信息中未配置电子表单Id，请检查逻辑！");
                return;
            }
            ...
            if (StringUtil.isBlank(eFormTableName)) {
                log.info("《船舶营业运输证》补发，办件办结后事件，未获取到电子Id：" + formId + "获取到表单表名！");
                return;
            }
            ...
            if (record == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，未根据到表单表名：" + eFormTableName + "、办件唯一标识：" + rowGuid + "获取到电子表单信息！");
                return;
            }
            ...
            if (certCbyyysz == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，未根据船舶营业运输证编号：" + cbyyyszbh + "查询到港航船舶营业运输证本地库数据！");
                return;
            }
            ...
            if (certinfo == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，未根据certrowguid：" + certrowguid + "查询到证照数据！");
                return;
            }
            ...
            if (certinfoExtension == null) {
                log.info("《船舶营业运输证》补发，办件办结后事件，未根据证照信息唯一标识：" + certinfo.getRowguid() + "查询到照面信息数据！");
