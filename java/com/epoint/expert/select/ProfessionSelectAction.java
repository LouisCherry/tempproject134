...
@RestController("professionselectaction")
@Scope("request")
public class ProfessionSelectAction extends BaseController
{
    ...
    @Autowired
    private ICodeItemsService codeitemServcie;
    @Autowired
    ...
    public List<Map<String, Object>> getUnChosenExpertList() {
        ...
        List<CodeItems> professionList = codeitemServcie.listCodeItemsByCodeName("专家库_评标专业");
        if (professionList != null && professionList.size() > 0) {
            Map<String, Object> map;
            for (CodeItems item : professionList) {
                map = new HashMap<>();
                map.put("id", item.getItemValue());
                map.put("text", item.getItemText());
    ...
    public void saveData(String ids, String names) {
        //先删除当前回避公司记录
        expertIRuleService.deleteObjectByInstanceguid(instanceGuid, RULEOBJECT_PROFESSION);
        ExpertIRule rule;
        if (StringUtil.isNotBlank(ids)) {
            ...
            for (int i = 0; i < idArray.length; i++) {
                ...
                rule.setObjecttype(RULEOBJECT_PROFESSION);
