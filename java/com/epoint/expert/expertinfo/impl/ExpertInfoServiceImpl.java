package com.epoint.expert.expertinfo.impl;

import java.util.List;
import java.util.Map;

import org.springframework.stereotype.Component;

import com.alibaba.dubbo.config.annotation.Service;
import com.epoint.database.peisistence.crud.impl.model.PageData;
import com.epoint.expert.expertinfo.api.IExpertInfoService;
import com.epoint.expert.expertinfo.api.entity.ExpertInfo;

/**
 * 专家表对应的后台service实现类
 * 
 * @author cqsong
 * @version [版本号, 2019-08-21 16:36:51]
 */
@Component
@Service
public class ExpertInfoServiceImpl implements IExpertInfoService
{
    /**
     * 
     */
    private static final long serialVersionUID = 9106960425090658469L;

    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(ExpertInfo record) {
        return new ExpertInfoService().insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int deleteByGuid(String guid) {
        return new ExpertInfoService().deleteByGuid(guid);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(ExpertInfo record) {
        return new ExpertInfoService().update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public ExpertInfo find(Object primaryKey) {
        return new ExpertInfoService().find(primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public ExpertInfo find(String sql, Object... args) {
        return new ExpertInfoService().find(args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<ExpertInfo> findList(String sql, Object... args) {
        return new ExpertInfoService().findList(sql, args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<ExpertInfo> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return new ExpertInfoService().findList(sql, pageNumber, pageSize, args);
    }
    
    @Override
    public List<ExpertInfo> findListByCondition(Map<String, String> conditionMap) {
        return new ExpertInfoService().findListByCondition(conditionMap);
    }

    @Override
    public List<String> getExpertGuidsByProfession(String professionGuids) {
        return new ExpertInfoService().getExpertGuidsByProfession(professionGuids);
    }

    @Override
    public List<String> getAvailiableExpertList() {
        return new ExpertInfoService().getAvailiableExpertList();
    }

    @Override
    public PageData<ExpertInfo> getPageByCondition(Map<String, String> conditionMap, int pageNumber, int pageSize,
            String sortField, String sortOrder) {
        return new ExpertInfoService().getPageByCondition(conditionMap, pageNumber, pageSize, sortField, sortOrder);
    }

    @Override
    public List<ExpertInfo> findList(Map<String, String> conditionMap, int pageNumber, int pageSize, String sortField,
            String sortOrder) {
        return new ExpertInfoService().findList(conditionMap, pageNumber, pageSize, sortField, sortOrder);
    }

    @Override
    public List<ExpertInfo> findList(Map<String, String> conditionMap) {
        return new ExpertInfoService().findList(conditionMap);
    }


    @Override
    public String getExpertFlowSn() {
      String serialNumber="000000";
     int maxFlowsn=new ExpertInfoService().getExpertFlowSn();
     //当前最大值加1
     serialNumber+=String.valueOf(maxFlowsn+1);
     //截取后6位
     serialNumber=serialNumber.substring(serialNumber.length()-6, serialNumber.length());
        return serialNumber;
    }

}
