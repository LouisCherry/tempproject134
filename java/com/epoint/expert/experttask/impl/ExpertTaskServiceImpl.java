package com.epoint.expert.experttask.impl;

import java.util.List;

import org.springframework.stereotype.Component;

import com.alibaba.dubbo.config.annotation.Service;
import com.epoint.expert.experttask.api.IExpertTaskService;
import com.epoint.expert.experttask.api.entity.ExpertTask;

/**
 * 专家关联事项表对应的后台service实现类
 * 
 * @author cqsong
 * @version [版本号, 2019-08-21 16:36:57]
 */
@Component
@Service
public class ExpertTaskServiceImpl implements IExpertTaskService
{
    /**
     * 
     */
    private static final long serialVersionUID = 953015616785927957L;

    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(ExpertTask record) {
        return new ExpertTaskService().insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int deleteByGuid(String guid) {
        return new ExpertTaskService().deleteByGuid(guid);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(ExpertTask record) {
        return new ExpertTaskService().update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public ExpertTask find(Object primaryKey) {
        return new ExpertTaskService().find(primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public ExpertTask find(String sql, Object... args) {
        return new ExpertTaskService().find(args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<ExpertTask> findList(String sql, Object... args) {
        return new ExpertTaskService().findList(sql, args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<ExpertTask> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return new ExpertTaskService().findList(sql, pageNumber, pageSize, args);
    }

    @Override
    public List<String> getExpertGuidsByTaskid(String taskid) {
        return new ExpertTaskService().getExpertGuidsByTaskid(taskid);
    }

    @Override
    public List<String> getTaskId(String expertinfoguid) {
        return new ExpertTaskService().getTaskId(expertinfoguid);
    }

    @Override
    public void deleteByExpertguid(String expertguid) {
        new ExpertTaskService().deleteByExpertguid(expertguid);
    }

}
