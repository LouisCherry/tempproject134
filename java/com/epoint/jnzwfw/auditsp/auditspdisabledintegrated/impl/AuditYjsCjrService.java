...
public class AuditYjsCjrService {
    ...
    public List<AuditYjsCjr> findList(String areacode) {
    	String sql = " select * from audit_yjs_cjr where rowguid not in (select cjrguid from audit_yjs_cjrinfo) and belong_district = ? limit 20";
    	return baseDao.findList(sql, AuditYjsCjr.class, areacode);
    }
    ...
    public AuditYjsCjrInfo getCjrInfoDetailByRowguid(String rowguid) {
    	String sql = "select * from audit_yjs_cjrinfo where cjrguid = ?";
        return baseDao.find(sql,AuditYjsCjrInfo.class, rowguid);
    }
    
    public Record getMzAreacodeByCjrAreacode(String CjrAreacode) {
    	String sql = "select mzareacode from cjr_mz_area where cjrareacode = ?";
        return baseDao.find(sql,Record.class, CjrAreacode);
    }
