...
public class GxbysjyDataSyncJob implements Job
{
    ...
    private void doService() {
        try {
            log.info("===============开始获取高校毕业生"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据===============");
            getSfDate();
            log.info("===============结束获取高校毕业生"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据===============");
        }
        catch (Exception e) {
        	e.printStackTrace();
            log.info("===============获取高校毕业生"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据出现异常===============");
    ...
    public void getSfDate() {
    	//开始获取高校毕业生当天事项所关联起来的办件信息
    	List<Record> baseinfos = ijnService.getGxbysjyProjectList();
    	log.info(EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"==============高校毕业生当天办件数据量为：==============="+baseinfos.size());
		if (baseinfos == null || baseinfos.size() == 0) {
			log.info("===============无需要推送高校毕业生当天办件数据，结束服务===============");
		} else {
			...
			for(Record record : baseinfos) {
				// String area = data.get("area");
                 ...
                 if (projectByproid != null ) {
                     log.info("===============高校毕业生该办件已同步，办件标识为===============" + record.getStr("projid"));
                     //修改标识位
                 ...
                 if (tasklists == null || tasklists.size() == 0) {
                     log.error("===============高校毕业生事项数据未查询到===============" + record.getStr("REGION_ID"));
                     //修改标识位
                 ...
                 rec.set("OperateUserName", "高校毕业生数据同步服务");
    ...
    public void getSfHjDate(String ORGBUSNO) {
        ...
        try {
    		...
    		if (baseinfos == null || baseinfos.size() == 0) {
    			log.info("===============无需要推送高校毕业生当天办件流程信息，对应的前置库标识为："+ORGBUSNO);
    		} else {
