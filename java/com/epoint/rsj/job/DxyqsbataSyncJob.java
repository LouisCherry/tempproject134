...
public class DxyqsbataSyncJob implements Job
{
    ...
    private void doService() {
        try {
            log.info("===============开始获取山东省大型仪器设备"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据===============");
            getSfDate();
            log.info("===============结束获取山东省大型仪器设备"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据===============");
        }
        catch (Exception e) {
        	e.printStackTrace();
            log.info("===============获取山东省大型仪器设备"+EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"当天的办件数据出现异常===============");
    ...
    public void getSfDate() {
    	//开始获取大型仪器设备当天事项所关联起来的办件信息
    	List<Record> baseinfos = ijnService.getGxbysjyProjectList();
    	log.info(EpointDateUtil.convertDate2String(new Date(), "yyyy-MM-dd")+"==============山东省大型仪器设备当天办件数据量为：==============="+baseinfos.size());
		if (baseinfos == null || baseinfos.size() == 0) {
			log.info("===============无需要推送山东省大型仪器设备当天办件数据，结束服务===============");
		} else {
			...
			for(Record record : baseinfos) {
				// String area = data.get("area");
                 ...
                 if (projectByproid != null ) {
                     log.info("===============山东省大型仪器设备该办件已同步，办件标识为===============" + record.getStr("projid"));
                     //修改标识位
                 ...
                 if (tasklists == null || tasklists.size() == 0) {
                     log.error("===============山东省大型仪器设备事项数据未查询到===============" + record.getStr("REGION_ID"));
                     //修改标识位
                 ...
                 rec.set("OperateUserName", "山东省大型仪器设备数据同步服务");
    ...
    public void getSfHjDate(String ORGBUSNO) {
        ...
        try {
    		...
    		if (baseinfos == null || baseinfos.size() == 0) {
    			log.info("===============无需要推送山东省大型仪器设备当天办件流程信息，对应的前置库标识为："+ORGBUSNO);
    		} else {
