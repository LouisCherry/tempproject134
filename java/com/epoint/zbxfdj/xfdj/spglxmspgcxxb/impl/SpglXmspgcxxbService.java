package com.epoint.zbxfdj.xfdj.spglxmspgcxxb.impl;

import com.epoint.core.dao.ICommonDao;
import com.epoint.zbxfdj.util.JDBCUtil;
import com.epoint.zbxfdj.xfdj.spglxmspgcxxb.api.entity.SpglXmspgcxxb;

import java.util.List;

/**
 * 项目审批过程信息表对应的后台service
 *
 * @author Anber
 * @version [版本号, 2022-12-22 21:30:33]
 */
public class SpglXmspgcxxbService {

    /**
     * 数据增删改查组件
     */
    protected ICommonDao highgoDao;

    public SpglXmspgcxxbService() {
        highgoDao = JDBCUtil.getHighGoJDBC();
    }

    /**
     * 更新数据
     *
     * @param record BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int updateSjsczt(String status, String id) {
        String sql = "update \"370800_SPGL_XMSPGCXXB\" set Sjsczt = ? where id = ? ";
        return highgoDao.execute(sql, status, id);
    }

    /**
     * 根据ID查找单个实体
     * <p>
     * 类<必须继承BaseEntity>
     *
     * @param primaryKey 主键
     * @return T extends BaseEntity
     */
    public SpglXmspgcxxb find(Object primaryKey) {
        return highgoDao.find(SpglXmspgcxxb.class, primaryKey);
    }

    /**
     * 查找单条记录
     *
     * @param sql  查询语句
     *             可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *             ;String.class;Integer.class;Long.class]
     * @param args 参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public SpglXmspgcxxb find(String sql, Object... args) {
        return highgoDao.find(sql, SpglXmspgcxxb.class, args);
    }

    /**
     * 查找一个list
     *
     * @param sql  查询语句 可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args 参数值数组
     * @return T extends BaseEntity
     */
    public List<SpglXmspgcxxb> findList(String sql, Object... args) {
        return highgoDao.findList(sql, SpglXmspgcxxb.class, args);
    }

    /**
     * 分页查找一个list
     *
     * @param sql        查询语句
     * @param pageNumber 记录行的偏移量
     * @param pageSize   记录行的最大数目 可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args       参数值数组
     * @return T extends BaseEntity
     */
    public List<SpglXmspgcxxb> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return highgoDao.findList(sql, pageNumber, pageSize, SpglXmspgcxxb.class, args);
    }

    /**
     * 查询数量
     *
     * @param sql  执行语句
     * @param args 参数
     * @return Integer
     */
    public Integer countSpglXmspgcxxb(String sql, Object... args) {
        return highgoDao.queryInt(sql, args);
    }

    /**
     * 查找所有SJSCZT=? list
     *
     * @param sjsczt
     * @return
     */
    public List<SpglXmspgcxxb> findListBySjsczt(Integer sjsczt) {
        return highgoDao.findList(
                "select ID,LSH,DFSJZJ,XZQHDM,SPBMBM,SPBMMC,SPRXM,SPSJ,SPDZLX,SPJG,SPYJ,SPJDMC,SJYXBS,SJWXYY,SJSCZT,SBYY from \"370800_SPGL_XMSPGCXXB\" where SJSCZT=? ORDER BY SPSJ ",
                SpglXmspgcxxb.class, sjsczt);
    }

}
