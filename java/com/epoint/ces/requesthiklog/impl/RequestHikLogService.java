...
public class RequestHikLogService {
    ...
    public List<String> getAllUserGh() {
        String sql = " select gonghao from audit_orga_member where BelongXiaQuCode  ='370830' and Is_KaoQin  = '1'";
        return baseDao.findList(sql, String.class);
    }

    public List<Record> getAllUser() {
        String sql = "select * from audit_orga_member a inner join hikrecord b on a.gonghao = b.gonghao where BelongXiaQuCode = '370830' and Is_KaoQin = '1' and date >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 DAY),'%Y-%c-%d') and time is not null";
