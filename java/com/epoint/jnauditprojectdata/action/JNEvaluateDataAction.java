 ...
 * @Version 1.0
 **/
@RestController("jnevaluatedataaction")
@Scope("request")
public class JNEvaluateDataAction extends BaseController {
     ...
     * 办件基本信息实体对象
     */
    private EvainstanceCk dataBean = new EvainstanceCk();
        ...
        if (modelall == null) {
            modelall = new DataGridModel<Record>() {
                ...
                public List<Record> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    List<Record> listNum = ijnAuditProject.findEvaluateDataList(dataBean, applydateStart, applydateEnd
                            , finishdateStart, finishdateEnd, areacode, searchOUGuidList, first, pageSize);
                    log.info("办件量查询结果：" + listNum);
//                    int listNumCount = ijnAuditProject.getEvaluateDataCount(areacode, searchOUGuidList);
                    // 查询总量
                    if(listNum==null || listNum.isEmpty()){
                        this.setRowCount(0 );
                    }else{
                        int listNumCount = ijnAuditProject.findEvaluateTotalDataCount(dataBean, applydateStart, applydateEnd
