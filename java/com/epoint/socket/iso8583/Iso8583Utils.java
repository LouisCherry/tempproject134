...
import com.epoint.socket.iso8583.DomainInfoForm;
import com.epoint.socket.iso8583.bean.EncodeType;
import com.epoint.socket.iso8583.bean.Iso8583Entity;
import com.epoint.socket.iso8583.bean.LengthType;
import com.epoint.socket.iso8583.utils.BitArray;
import com.epoint.socket.iso8583.utils.ByteUtils;
import com.epoint.socket.iso8583.utils.Converts;
import com.epoint.socket.socketWriter;
...
public class Iso8583Utils {
	 ...
	 * @throws IOException
	 */
	public byte[] packageIso8583Datas(Map<Integer, String> content,boolean isUseExtendedBitMap,String foot) throws IOException {
		...
		//7、计算总包的长度 2字节长度+ 11字节TPDU
		int resultLen = output.toByteArray().length + 11 + 2;
		...
		socketWriter s = new socketWriter();

		byte[] tpdu = { 0x60, 0x00, 0x00, 0x00, 0x03, 0x60, 0x31, 0x00, 0x11,
				0x43, 0x00 };
		//9、写入tpdu
		result.write(s.hexStringToBytes("6000840089"));
		//写入消息头
		result.write(s.hexStringToBytes("603000320106"));
		//写入消息类型
		result.write(s.hexStringToBytes(foot));
		//10、写入域和位图数据
