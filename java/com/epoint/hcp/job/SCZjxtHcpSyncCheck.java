 ...
 * @since [产品/模块版本]
 */
public class SCZjxtHcpSyncCheck implements Callable<String>
{
	...
	IJnService iAuditProjectZjxtService = ContainerFactory.getContainInfo().getComponent(IJnService.class);
    /**
     * 错误数据rowguid
     */
    ...
    class Task implements Runnable
    {
        ...
        public void run() {
            try {
            	...
            	log.info("当前月份："+month);
                List<Record> projects = iHcpService.getAuditProjectZjxtList(start, size,month);
                ...
                try {
                	for (Record project : projects) {
						...
						String s = turnhcpevaluate(project, 1,"自建系统数据推送",newserviceTime,month);
						String status = "1";
						if("1".equals(s)) {
							status = "1";
						}else {
						...
						iHcpService.updateAuditProjectZjxt(status, project.get("rowguid"));
						EpointFrameDsManager.commit();
     ...
     * @see [类、类#方法、类#成员]
     */
    public String turnhcpevaluate(Record auditProject, int serviceNumber,String servicename,String newserviceTime,String month) {
		AuditTask auditTask = iAuditTask.getAuditTaskByGuid(auditProject.get("taskguid"), true).getResult();
		//ExternalProjectInfoExt auditProjectExt = extService.getExternalProjectByProjectguid(auditProject.get("rowguid"),month);
		//Record RtrRecord = iAuditProjectZjxtService.getAuditProjectZjxtByRowguid(auditProject.get("rowguid"));
		if (auditTask != null && auditProject != null) {
	            ...
	            String ouguid = auditProject.get("ouguid");
	            ...
	            json.put("areaCode", auditProject.getStr("areacode").replace("370882", "370812")+"000000");
	            json.put("taskName", auditTask.getTaskname());
	            json.put("projectNo", auditProject.get("flowsn"));
	            String proStatus = serviceNumber+"";
	            String acceptdate = EpointDateUtil.convertDate2String(auditProject.get("acceptuserdate"), "yyyy-MM-dd HH:mm:ss");
	            json.put("proStatus", proStatus);
	            json.put("orgcode", auditProject.getStr("areacode").replace("370882", "370812") + "000000_" + deptcode);
	            json.put("ouguid",ouguid);
	            // json.put("orgcode", "370900000000_11370900004341048Y");
	            json.put("orgName", auditProject.get("ouname"));
	            json.put("acceptDate", acceptdate);
	            Integer applyertype = auditProject.get("applyertype");
	            if (applyertype == 10) {
	                applyertype = 2;
	            }
	            ...
	            json.put("userName", StringUtil.isNotBlank(auditProject.get("applyername")) ?auditProject.getStr("applyername").trim() : "无");
	            json.put("userPageType", "111");
	            json.put("proManager", StringUtil.isNotBlank(auditProject.get("acceptusername")) ? auditProject.getStr("acceptusername").trim() : "无");
	            json.put("certKey", "0");
	            json.put("certKeyGOV", "0");
	            ...
	            if (3 == Integer.parseInt(proStatus)) {
	                json.put("resultDate",
	                        EpointDateUtil.convertDate2String(auditProject.get("banjiedate"), "yyyy-MM-dd HH:mm:ss"));
	            }
	            json.put("tasktype", "1");
	            json.put("mobile","0");
	            json.put("deptCode", deptcode);
	            json.put("projectName", "关于" + auditProject.getStr("applyername").trim() + auditTask.getTaskname() + "的业务");
	            json.put("creditNum",  "0");
	            // 默认证照类型为身份证
	            ...
	            if (StringUtil.isNotBlank(resultsign)) {
	                ...
	                if ("1".equals(custom.getString("code"))) {
	                	turnEvaluate(taskType,"37"+auditProject.get("flowsn"),StringUtil.isNotBlank(auditProject.get("applyername")) ? auditProject.getStr("applyername").trim() : "无", String.valueOf(serviceNumber),month);
	    				return "1";
	                }
	                else {
	                	turnEvaluate(taskType,"37"+auditProject.get("flowsn"),StringUtil.isNotBlank(auditProject.get("applyername")) ? auditProject.getStr("applyername").trim() : "无", String.valueOf(serviceNumber),month);
	                	log.info("自建系统保存办件服务数据失败:" + auditProject.get("flowsn")+"原因："+resultsign);
	                	return "0";
    ...
    private void turnEvaluate(String taskType,String projectno,String userName,String serviceNumber,String month) {
    	log.info("开始推送评价数据,flowsn:"+projectno);
