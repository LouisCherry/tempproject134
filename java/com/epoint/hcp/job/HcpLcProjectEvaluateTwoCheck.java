 ...
 * @since [产品/模块版本]
 */
public class HcpLcProjectEvaluateTwoCheck implements Callable<String> {
    ...
    public HcpLcProjectEvaluateTwoCheck() {
        super();
    }
    ...
    class Task implements Runnable {
        ...
        public void run() {
            try {
                List<lcprojecttwo> projects = iHcpService.getLcEvaluateSbTwoList(start, size);
                EpointFrameDsManager.begin(null);
                try {
                    for (lcprojecttwo project : projects) {
                        project = iHcpService.getlcprojecttwoByRowguid(project.getRowguid());
                        String mobile = project.getContactmobile();
                        //没有手机号的直接不传
                        if (StringUtil.isBlank(mobile)) {
                            iHcpService.updateLcProjecttwo("2", project.getRowguid());
                        } else {
                            //有手机号的检测是否满足手机号校验
                            boolean flag = isValid(mobile);
                            if (flag) {
                                Date date = new Date();
                                Calendar c = new GregorianCalendar();
                                c.setTime(date);//设置参数时间
                                c.add(Calendar.HOUR, -3);//把日期往后增加SECOND 秒.整数往后推,负数往前移动
                                ...
                                c1.add(Calendar.HOUR, -2);//把日期往后增加SECOND 秒.整数往后推,负数往前移动
                                ...
                                c2.add(Calendar.HOUR, -1);//把日期往后增加SECOND 秒.整数往后推,负数往前移动
                                date2 = c2.getTime(); //这个时间就是日期往后推一天的结果
                                String newserviceTime2 = EpointDateUtil.convertDate2String(date2, "yyyy-MM-dd HH:mm:ss");

                                String s = turnhcpevaluate(project, 1, "提交申请信息", newserviceTime, "1");
                                EpointFrameDsManager.commit();
                                String ss = turnhcpevaluate(project, 2, "待受理", newserviceTime1, "2");
                                EpointFrameDsManager.commit();
                                String sss = turnhcpevaluate(project, 3, "出证办结", newserviceTime2, "3");
                                EpointFrameDsManager.commit();
                                String status = "1";
                                if ("1".equals(s)) {
                                    status += "1";
                                ...
                                iHcpService.updateLcProjecttwo(status, project.getRowguid());
                                EpointFrameDsManager.commit();
                            } else {
                                iHcpService.updateLcProjecttwo("2", project.getRowguid());
                            }
     ...
     * [推送好差评办件服务数据]
     */
    public String turnhcpevaluate(lcprojecttwo auditProject, int serviceNumber, String servicename, String newserviceTime, String type) {
    ...
    private void turnEvaluate(String taskType, String areacode, String projectno, String userName, String serviceNumber) {
        ...
        Record r = new Record();
        r.setSql_TableName("evainstancetwo");
    ...
    public static boolean isValid(String phone) {
        String regex = "^0{0,1}(13[0-9]|16[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9]|19[0-9])[0-9]{8}$";
