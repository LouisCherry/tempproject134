...
public class Projectdown2 implements Job
{
    transient Logger log = LogUtil.getLog(Projectdown2.class);
    ...
    private IAttachService attachService = ContainerFactory.getContainInfo().getComponent(IAttachService.class);
        ...
        try {
            ...
            List<AuditProject> projectList = service.getProjectbytsflag1();
                ...
                if (recordList != null && recordList.size() > 0) {
                    for (Record record : recordList) {
                        ...
                        if (intstatus>=90) {
                            ...
                            EpointFrameDsManager.commit();
                            log.info("网厅对应办件修改成功了1 " + i + "条办件");
                           
                            log.info("取消办件挂机状态成功1 " + project.getFlowsn());
                            //对办理结果材料进行更新
