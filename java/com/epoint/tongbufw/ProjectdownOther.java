...
public class ProjectdownOther implements Job
{
    transient Logger log = LogUtil.getLog(ProjectdownOther.class);
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            ...
            //程序块运行时间
            log.info("ProjectOtherTime运行时间为：" + (endTime - startTime)/1000 + "秒");
    ...
    private void doService() {
        try {
            BasicInfoSyncGgService service = new BasicInfoSyncGgService();
            List<AuditProject> projectList = service.getProjectbyOther();
            //办件状态更新办件办结信息
            log.info("开始同步 其他所有类型办件流程");
                ...
                if (recordList != null && recordList.size() > 0) {
                    for (Record record : recordList) {
                        ...
                        if (intstatus>=90) {
                            ...
                            //办件状态更新办件办结信息
                            log.info("开始同步 其他所有类型办件结果 " + flowsn);
                            int BANJIERESULT = 40;
