...
public class ProjectdownGgfwService
{
    transient Logger log = LogUtil.getLog(ProjectdownGgfwService.class);
    ...
    public List<AuditProject> getProjectbytsflag() {
        String sql = "select a.* from audit_project a join audit_task b on a.taskguid = b.rowguid WHERE ISSYNACWAVE='1' AND STATUS<> 90 and STATUS<> 99 and (is_lczj is null or is_lczj='4') and b.shenpilb = '11' ORDER BY ACCEPTUSERDATE DESC limit 150";
        List<AuditProject> list = commonDaoTo.findList(sql, AuditProject.class);
        return list;
    }

    public List<Record> getprocbyflowsn(String flowsn) {
        String sql  = "select * from APPROVE_BUSINESS_COURSE_GGFW t where Receive_number= '"+flowsn+"' and flag is null and rownum <= 50 ORDER BY SEND_TIME ASC";
    ...
    public int updateflag(String sn) {
        String sql = "update APPROVE_BUSINESS_COURSE_GGFW set flag='1' where id=?";
