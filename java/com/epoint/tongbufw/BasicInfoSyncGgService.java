...
public class BasicInfoSyncGgService
{
    transient Logger log = LogUtil.getLog(BasicInfoSyncGgService.class);
    ...
    public List<AuditProject> getProjectbytsflag() {
        String sql = "select a.* from audit_project a join audit_task b on a.taskguid = b.rowguid WHERE ISSYNACWAVE='1' AND STATUS<> 90 and STATUS<> 99 and (is_lczj is null or is_lczj='4') and b.shenpilb = '10' ORDER BY ACCEPTUSERDATE DESC limit 150";
        List<AuditProject> list = commonDaoTo.findList(sql, AuditProject.class);
        return list;
    }
    
    public List<AuditProject> getProjectbyOther() {
        String sql = "select a.* from audit_project a join audit_task b on a.taskguid = b.rowguid WHERE ISSYNACWAVE='1' AND STATUS<> 90 and STATUS<> 99 and (is_lczj is null or is_lczj='4') AND b.shenpilb not in ('10','11','01') ORDER BY ACCEPTUSERDATE DESC limit 150";
        List<AuditProject> list = commonDaoTo.findList(sql, AuditProject.class);
        return list;
    }

    public List<Record> getprocbyflowsn(String flowsn) {
        String sql  = "select * from APPROVE_BUSINESS_COURSE_QTQL t where Receive_number= '"+flowsn+"' and flag is null and rownum <= 50 ORDER BY SEND_TIME ASC";
    ...
    public int updateflag(String sn) {
        String sql = "update APPROVE_BUSINESS_COURSE_QTQL set flag='1' where id=?";
    ...
    public Record getProjectByFlowsn(String flowsn){
        String sql = "SELECT p.rowguid as projectguid, p.status ,s.rowguid as spareguid from audit_project p inner join audit_project_sparetime s on p.rowguid = s.projectguid "
                + " WHERE flowsn =? and status < 90";
        Record project = commonDaoTo.find(sql, Record.class, flowsn);
        return project;
    }
    
    public int updatePauseByGuid(String projectguid,String spareguid, int pause) {
        String sql1 = "update audit_project set IS_PAUSE="+pause+",OperateUserName='浪潮自动挂起',OperateDate=now() where rowguid=?";
        String sql2 = "update audit_project_sparetime set PAUSE="+pause+" where rowguid=?";
        int result = 0;
        if(StringUtil.isNotBlank(projectguid)){
            result += commonDaoTo.execute(sql1, projectguid);
        }
        if(StringUtil.isNotBlank(spareguid)){
            result += commonDaoTo.execute(sql2, spareguid);
        }
        return result;
    }
