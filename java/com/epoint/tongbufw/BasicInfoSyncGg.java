...
public class BasicInfoSyncGg implements Job
{
    transient Logger log = LogUtil.getLog(BasicInfoSyncGg.class);
    ...
    private void doService() {
        try {
        	ProjectdownGgfwService service = new ProjectdownGgfwService();
            List<AuditProject> projectList = service.getProjectbytsflag();
            //办件状态更新办件办结信息
            log.info("开始同步 公共服务类型办件流程");
                ...
                if (recordList != null && recordList.size() > 0) {
                    for (Record record : recordList) {
                        ...
                        if (intstatus>=90) {
                            ...
                            //办件状态更新办件办结信息
                            log.info("开始同步 其他类型办件结果 " + flowsn);
                            int BANJIERESULT = 40;
                            project.setBanjiedate(record.getDate("SEND_TIME"));
                            project.setStatus(intstatus);
                            project.setBanjieusername(record.getStr("USER_NAME"));
                            project.setBanjieresult(BANJIERESULT);
                ...
                if(StringUtil.isNotBlank(gq.getStr("RECEIVE_NUMBER")) 
                    ...
                    if(project!=null){
                        if("1".equals(gq.getStr("ACTIVE"))){
                            service.updatePauseByGuid(project.getStr("projectguid"), project.getStr("spareguid"), 1);
                        }else if("0".equals(gq.getStr("ACTIVE"))){
                            service.updatePauseByGuid(project.getStr("projectguid"), project.getStr("spareguid"), 0);
                        }
    ...
    public void getWtResult(String projectGuid, String flowsn) {
        try {
                ...
                //程序块运行时间
                log.info("ProjectdownGgfwTime运行时间为：" + (endTime - startTime)/1000 + "秒");
                // 8、定义返回JSON对象
