 ...
 */
@RestController("jnauditonlineconsultlistaction")
@Scope("request")
public class JNAuditOnlineConsultListAction extends BaseController
{
    /**
     * 
     */
     ...
     * 咨询投诉实体对象
     */
    private AuditOnlineConsult dataBean;

    /**
     * 表格控件model
     */
    private DataGridModel<AuditOnlineConsult> model;
    ...
    public void deleteSelect() {
        ...
        for (String sel : select) {
            // 删除的时候要清空和部门扩展表的关系
            AuditOnlineConsult consult = onlineConsult.getConsultByRowguid(sel).getResult();
    ...
    public DataGridModel<AuditOnlineConsult> getDataGridData() {
        // 获得表格对象
        if (model == null) {
            model = new DataGridModel<AuditOnlineConsult>()
            {
                /**
                 * 
                 */
                ...
                @Override
                public List<AuditOnlineConsult> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    //设置类型  1为咨询  2为投诉
                    if (StringUtil.isNotBlank(consulttype) && ZwfwConstant.CONSTANT_STR_ONE.equals(consulttype)) {
                        sql.eq("consulttype", consulttype);
                        if(StringUtil.isBlank(department)){
                              sql.isBlankOrValue("ouguid", "");                            
                        }
                    }
                    if(StringUtil.isNotBlank(consulttype) && ZwfwConstant.CONSTANT_STR_TWO.equals(consulttype)){
                        sql.eq("consulttype", consulttype);
                        if(StringUtil.isNotBlank(isou) && "true".equals(isou)){
                            sql.isNotBlank("ouguid");
                        }
                    }
                    //中心
                    ...
                    PageData<AuditOnlineConsult> pageData = getPageData(sql.getMap(), first, pageSize,
                            sortField, sortOrder);
     ...
     * @return
     */
    public PageData<AuditOnlineConsult> getPageData(Map<String, String> conditionMap, Integer first, Integer pageSize,
            String sortField, String sortOrder) {
        PageData<AuditOnlineConsult> pageData = new PageData<AuditOnlineConsult>();
        List<AuditOnlineConsult> consultList = onlineConsult
                .selectConsultByPage(conditionMap, first, pageSize, sortField, sortOrder).getResult();
        for (AuditOnlineConsult consult : consultList) {
