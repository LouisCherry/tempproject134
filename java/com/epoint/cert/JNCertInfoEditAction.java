 ...
 */
@RestController("jncertinfoeditaction")
@Scope("request")
public class JNCertInfoEditAction extends BaseController {
     ...
     * 日志
     */
    private Logger log = LogUtil.getLog(JNCertInfoEditAction.class);
...
//        }
    ...
    public void generateCert(String parentguidList, String cliengguids, String isGenerateCopy) throws ParseException {
        ...
        sqlConditionUtil.eq("Certrowguid", oldcertguid);
        List<AuditProject> auditProjects = iJNAuditProject.getAuditProjectListByCondition(sqlConditionUtil.getMap()).getResult();
        AuditProject auditProject = null;
        if (auditProjects != null && auditProjects.size() > 0) {
            auditProject = auditProjects.get(0);
            setCertValue(parentguidList, cliengguids, jsonObject, auditProject.getOuguid());
            certinfo.set("projectguid", auditProject.getRowguid());
        }else{
            log.info("guid:"+guid);
            log.info("oldguid:"+oldcertguid);
        }
        certinfo.set("oldguid",oldcertguid);
        ...
        if ("生成证照成功！".equals(msg)) {
            if(auditProject!=null){
                String certrowguids = certinfo.getRowguid();
//                if (StringUtil.isBlank(certrowguids)) {
    ...
    public void generateCertGz(String parentguidList, String cliengguids) throws ParseException {
        ...
        try {
            ...
            if (auditProjects != null && auditProjects.size() > 0) {
                ...
                log.info("guid"+guid);
                log.info("oldcertguid"+oldcertguid);
            }
...
//                        }
    ...
    public void setCertValue(String params, String cliengguids, JSONObject cliengguidsObject, String ouguid) {
        String areacode = "";
        if (ZwfwUserSession.getInstance().getCitylevel() != null
                && (Integer.parseInt(ZwfwUserSession.getInstance().getCitylevel()) > Integer
                .parseInt(ZwfwConstant.AREA_TYPE_XQJ))) {
        ...
        if (StringUtil.isNotBlank(getViewData("rowguid"))) {
            ...
            try {
                ...
                if (cliengguidsObject != null && !cliengguidsObject.isEmpty()) {
                    ...
                    if (com.epoint.common.util.ValidateUtil.isNotBlankCollection(keySet)) {
    ...
    public void setSubExtensionJson(JSONObject jsonObject, Record record) {
        ...
        if (com.epoint.common.util.ValidateUtil.isNotBlankCollection(keySet)) {
            for (String parentguid : keySet) {
                ...
                JSONArray jsonArray = new JSONArray();
                if (com.epoint.common.util.ValidateUtil.isNotBlankCollection(extensionList)) {
    ...
    public void pushbigdata(){
        ...
        if (auditProjects != null && !auditProjects.isEmpty()) {
            ...
            if (auditProject != null && auditTask != null) {
                ...
                JNAuditHandleControlAction action = ContainerFactory.getContainInfo()
                        .getComponent(JNAuditHandleControlAction.class);
                action.banjiePushBigDataMethod(auditProject, auditTask);
            }
            addCallbackParam("msg","推送成功");
        }
        else {
            log.info("guid"+guid);
            log.info("oldcertguid"+oldcertguid);
            addCallbackParam("msg","证照关联的办件不存在");
        }
    }
