...
import com.epoint.database.jdbc.config.EpointDataSource;
 ...
 */
@RestController("yjscertinfoaddaction")
@Scope("request")
public class YjsCertInfoAddAction extends BaseController
{
     ...
     * 日志
     */
    private Logger log = LogUtil.getLog(YjsCertInfoAddAction.class);
            ...
            if (!isPostback()) {
             ...
             // 渲染基本信息   
                ...
                if(auditSpISubapp != null) {
                		Record record = iCxBusService.getDzbdDetail("formtable20210902171645",auditSpISubapp.getRowguid());
                    	if (record != null) {
                    		Date startdate = new Date();
                    		String ksrq = EpointDateUtil.convertDate2String(startdate,EpointDateUtil.DATE_FORMAT);
                    		
                    		addCallbackParam("xingming", record.getStr("cyryxm"));
                    		addCallbackParam("zhuzhi", record.getStr("zz"));
                    		addCallbackParam("zhenghao", record.getStr("zjhm"));
                    		addCallbackParam("xingbie", record.getStr("xb"));
                    		addCallbackParam("zhunjcx", record.getStr("zjcx"));
                    		addCallbackParam("chusrq", record.getStr("csrq"));
                    		addCallbackParam("zhaopian", record.getStr("zzlbjdj"));
                    		addCallbackParam("guoji", record.getStr("gj"));
                    		addCallbackParam("cslzrq", ksrq);
                    		addCallbackParam("youxqsrq", ksrq);
                    		
                    		Calendar calendar = new GregorianCalendar();
                            calendar.setTime(startdate);
                            calendar.add(Calendar.YEAR, 6); //把日期往后增加一年，整数往后推，负数往前移
                            calendar.add(Calendar.DAY_OF_MONTH, -1); //把日期向前调整一天
                            Date enddate = calendar.getTime();
                            String jsrq = EpointDateUtil.convertDate2String(enddate, EpointDateUtil.DATE_FORMAT);
                            addCallbackParam("yxjsrq", jsrq);
...
//        certinfo.setCertawarddept(certinfo.getCertawarddept().replace(",", "^"));
        ...
        if (StringUtil.isNotBlank(getViewData("rowguid"))) { // 保存
            ...
            certinfo.setCertownername(FormatUtil.getEncryptSM4ToData("一件事" , certcatalogid , ""));
            certinfo.setCertownerno(FormatUtil.getEncryptSM4ToData("YJS" , certcatalogid , ""));
...
//            }
    ...
    public void generateCert(String parentguidList, String cliengguids, String isGenerateCopy, String ouguids) throws ParseException {
        ...
        certinfo.set("projectguid", getRequestParameter("biguid"));
