package com.epoint.zoucheng.device.infopub.infopubmaterial.impl;
import java.util.List;

import org.springframework.stereotype.Component;

import com.alibaba.dubbo.config.annotation.Service;
import com.epoint.zoucheng.device.infopub.infopubmaterial.api.IInfopubMaterialService;
import com.epoint.zoucheng.device.infopub.infopubmaterial.api.entity.InfopubMaterial;
/**
 * 素材表对应的后台service实现类
 * 
 * @author why
 * @version [版本号, 2019-09-16 19:57:24]
 */
@Component
@Service
public class InfopubMaterialServiceImpl implements IInfopubMaterialService
{
    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(InfopubMaterial record) {
        return new InfopubMaterialService().insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int deleteByGuid(String guid) {
        return new InfopubMaterialService().deleteByGuid(guid);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(InfopubMaterial record) {
        return new InfopubMaterialService().update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public InfopubMaterial find(Object primaryKey) {
       return new InfopubMaterialService().find(primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public InfopubMaterial find(String sql, Object... args) {
        return new InfopubMaterialService().find(args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<InfopubMaterial> findList(String sql, Object... args) {
       return new InfopubMaterialService().findList(sql,args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<InfopubMaterial> findList(String sql, int pageNumber, int pageSize, Object... args) {
       return new InfopubMaterialService().findList(sql,pageNumber,pageSize,args);
    }

    @Override
    public int getMaterialCount(String name, String viewData) {
        // TODO Auto-generated method stub
        return new InfopubMaterialService().getMaterialCount(name, viewData);
    }

}
