...
@RestController("zcauditznsbquestioninfoaddaction")
@Scope("request")
public class ZCAuditZnsbQuestioninfoAddAction extends BaseController
{
    /**
     * 
     */
    ...
    public void add() {
        ...
        final int two = 2;
        if (questiondetailService.getCountByGuid(rowguid) < two) {
            addCallbackParam("msg", "选项需要大于等于两个！");
        }
        else {
    ...
    public void saveAll(String changedata) {
        ...
        if (!newList.isEmpty()) {
            // 正确选项数量校验
            /*
             * int count = 0; for (Record record : newList) { if
             * ("1".equals(record.getStr("answer"))) { count++; } } if (count ==
             * 0) { addCallbackParam("msg", "请至少勾选一个正确选项！"); return; } else if
             * (!"1".equals(dataBean.getQuestiontype())) { // 非多选题 if (count >
             * 1) { addCallbackParam("msg", "非多选题只能有一个正确选项！"); return; } }
             */
            for (Record record : newList) {
                if (StringUtil.isBlank(record.getStr("rowguid"))) {
                    if (StringUtil.isBlank(record.getStr("answeroption"))
                    ...
                    else if (questiondetailService.getCountByName(record.getStr("answeroption"), rowguid,
                        ...
                        questiondetail.setQuestionguid(rowguid);
                    ...
                    if (StringUtil.isBlank(record.getStr("rowguid"))) {
                        ...
                        if (questiondetailService.getCountByName(record.getStr("answeroption"), rowguid,
                                record.getStr("rowguid")) > 0) {
                            addCallbackParam("msg", "选项名称重复，请重新添加！");
                        }
                        else {
