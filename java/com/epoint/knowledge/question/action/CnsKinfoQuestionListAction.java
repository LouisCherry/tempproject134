 ...
 */
@RestController("cnskinfoquestionlistaction")
@Scope("request")
public class CnsKinfoQuestionListAction extends BaseController
{
        ...
        if (model == null) {
            model = new DataGridModel<CnsKinfoQuestion>()
            {
                ...
                public List<CnsKinfoQuestion> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    conditionMap.put("ifnull(iskinfo,'0')<>", "1");
                    ...
                    if (StringUtil.isNotBlank(startDate)) {
                        conditionMap.put("to_char(ASKTIME,'yyyy-mm-dd')GTE",
                                EpointDateUtil.convertDate2String(
                                        EpointDateUtil.convertString2Date(startDate, EpointDateUtil.DATE_TIME_FORMAT),
                                        EpointDateUtil.DATE_FORMAT));
                    }
                    if (StringUtil.isNotBlank(endDate)) {
                        conditionMap.put("to_char(ASKTIME,'yyyy-mm-dd')LTE",
                                EpointDateUtil.convertDate2String(
                                        EpointDateUtil.convertString2Date(endDate, EpointDateUtil.DATE_TIME_FORMAT),
                                        EpointDateUtil.DATE_FORMAT));
                    }
                    ...
                    List<CnsKinfoQuestion> list = kinfoQuestionService.getRecordPage(
                            "rowguid,questioncontent,asktime,kinfocategoryname,deptname,answerstatus,pviguid", conditionMap,
