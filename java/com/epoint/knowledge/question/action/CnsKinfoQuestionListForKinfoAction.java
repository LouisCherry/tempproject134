 ...
 */
@RestController("cnskinfoquestionlistforkinfoaction")
@Scope("request")
public class CnsKinfoQuestionListForKinfoAction extends BaseController
{

    /**
     * 
     */
    private static final long serialVersionUID = 3814110460442198482L;
    ...
    WorkflowWorkItemService9 workflowWorkItemService9=new WorkflowWorkItemService9();
    /**
     * 知识库提问表实体对象
     */
        ...
        if (model == null) {
            model = new DataGridModel<CnsKinfoQuestion>()
            {
                ...
                public List<CnsKinfoQuestion> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    conditionMap.put("ASKPERSONGUID=", userSession.getUserGuid());
                    conditionMap.put("iskinfo = ", "1");
                    ...
                    List<CnsKinfoQuestion> list = kinfoQuestionService.getRecordPage(
                            "rowguid,questioncontent,asktime,kinfocategoryname,deptname,answerstatus,pviguid,kinfoguid", conditionMap,
                            first, pageSize, "asktime", "desc");
                    for (CnsKinfoQuestion c : list) {
                        String  kinfoguid= c.getKinfoguid();
                        if(StringUtil.isNotBlank(kinfoguid)){
                            String sql = "select kname from cns_kinfo where rowguid='"+kinfoguid+"'";
                            String kname = CommonDao.getInstance().find(sql, String.class, null);
                            c.put("kinfoname", kname);
                        }
    ...
    public List<SelectItem> getAnsStatusModel() {
        answerstatusModel = DataUtil.convertMap2ComboBox(
                (List<Map<String, String>>) CodeModalFactory.factory("下拉列表", "提问答复状态", null, false));
