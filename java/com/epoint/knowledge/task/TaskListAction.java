 ...
 */
@RestController("tasklistaction")
@Scope("request")
public class TaskListAction extends BaseController
{
    /**
     * 
     */
    ...
    private List<SelectItem> isjyModel;
    ...
    public String getShenpilbcode(String shenpi) {
        List<Map<String, String>> list = CodeModalFactory.factory(EpointKeyNames9.CHECK_SELECT_GROUP, "审批类别", null, false);
    ...
    public List<SelectItem> getIsjyModel() {
        if (isjyModel == null) {
            isjyModel = DataUtil.convertMap2ComboBox((List<Map<String, String>>) CodeModalFactory
                    .factory(EpointKeyNames9.CHECK_SELECT_GROUP, "禁用类型", null, false));
        }
        return this.isjyModel;
    }
                ...
                public List<AuditTask> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if (StringUtil.isNotBlank(leftTreeNodeGuid)) {
                        ...
                        if (auditOrgaArea != null) {
                        	//取辖区下窗口部门
                        	List<FrameOu> frameOus = frameOu.getWindowOUList(auditOrgaArea.getXiaqucode(), true).getResult();
                            String ouGuids = "'";
                    ...
                    PageData<AuditTask> pageData = auditTaskBasicImpl
                            .getAuditTaskPageData(sql.getMap(), first, pageSize, sortField, sortOrder).getResult();
