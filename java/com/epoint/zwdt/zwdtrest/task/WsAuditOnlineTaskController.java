 ...
 */
@RestController
@RequestMapping("/wszwdtTask")
public class WsAuditOnlineTaskController
{
                ...
                if (auditTask != null) {
                    ...
                    if ("1".equals(isNeedAll)) {
                        ...
                        if(StringUtil.isNotBlank(auditTask.getCharge_flag())) {
                            taskBasicJson.put("chargeflag",  iCodeItemsService.getItemTextByCodeName("是否",String.valueOf(auditTask.getCharge_flag()))); // 是否收费
                        }else {
                        ...
                        if(StringUtil.isNotBlank(auditTask.getStr("ORG_DECIDE_EMAIL"))) {
                            ...
                            taskBasicJson.put("orgdecideemail", "无");
                        }
                        ...
                        String limitnum = auditTaskExtension.getStr("LIMIT_NUMBER");
                        ...
                        taskBasicJson.put("webapplydepth", WEBAPPLYDEPTH);
                        ...
                        if(StringUtil.isNotBlank(banliliucheng)) {
                             liucheng = syncbanliliucheng(banliliucheng);// 办理流程
                        }
                        ...
                        if(StringUtil.isNotBlank(addresstime)) {
                        	acceptaddinfos = syncbanlididiansj(addresstime);// 办理流程
                        }
                        if (auditTaskExtension != null) {
                            ...
                            //通办范围
                            taskBasicJson.put("operationscope", auditTaskExtension.getOperationscope());
                            //是否支持物流快递
                            taskBasicJson.put("isexpress", auditTaskExtension.getIf_express());
                            			 ...
                            			 for (int i=0;i<zhongjies.size();i++) {
                                			 ...
                                			 for (int j=0;j<typeAndReqs.size();j++) {
                                				 ...
                                				 for (int k=0;k<typereq.size();k++) {
                                					 typeAndReq += iCodeItemsService.getItemTextByCodeName("中介机构服务类别", typereq.getString(k)) + "：";
                                				 }
                                				 typeAndReq += "。";
                                			 }
                                			 ...
                                			 zjie.set("intermediaryServiceOrganization", zhongjie.getString("intermediaryServiceOrganization"));
                                			 
                                			 String intermediaryServicesCharges = zhongjie.getString("intermediaryServicesCharges");
                                			 ...
                                			 JSONArray intermediateResults = zhongjie.getJSONArray("intermediateResults");
                                			 
//                                			 zjie.set("intermediateResults", zhongjie.getString("intermediateResults"));
                        	...
                        	//是否中介服务
                             if (StringUtil.isNotBlank(auditTaskExtension.getStr("IS_AGENCYORGAN")) && !"0".equals(auditTaskExtension.getStr("IS_AGENCYORGAN"))) {
                                   ...
                                   if(StringUtil.isNotBlank(serverprojectinfo) && serverprojectinfo.contains("version")) {
                                 	  taskBasicJson.put("isagencyorgan", serverprojectinfo.substring(97)
                                       		 .replace("<CHANGE_FLAG>0</CHANGE_FLAG>", "").replace("<SERVEPROJECTS>", "").replace("<SERVEPROJECT>", "")
                                       	 	 .replace("<NAME>", "<br>中介服务项目名称：").replace("<BASIC_LAW>", "<br>法律依据：").replace("<RESULT_NAME>", "<br>结果名称：")
                                       	 	 .replace("<REMARK>", "<br>联系方式：") .replace("</REMARK>", "").replace("</RESULT_NAME>", "")
                                       	 	 .replace("</NAME>", "").replace("</BASIC_LAW>", "").replace("</SERVEPROJECT>", "")
                                       	 	 .replace("</SERVEPROJECTS>", "").replace("</DATAAREA>", "").replace("PROJECT>", ""));
                                   }else {
                            ...
                            if (s != null && s.length() > 30) {
                                ...
                                //办理结果类型
                                taskBasicJson.put("resulttype",  strs[1].substring(0, strs[1].length() - 1).replace("^", "、").replace("10", "证照").replace("20", "批文").replace("30", "其他").replace("99", "无"));
                                //送达方式
                                taskBasicJson.put("servicemode",  strs[2].substring(0, strs[2].length() - 1).replace("^", "、").replace("1", "窗口领取").replace("2", "公告送达").replace("3", "邮寄送达").replace("4", "网站下载").replace("5", "其他"));
                            ...
                            if(StringUtil.isNotBlank(auditTaskExtension.getStr("HANDLE_FORM"))) {
                                ...
                                taskBasicJson.put("handleform", auditTaskExtension.getStr("HANDLE_FORM").replace("^", "、").replace("0", "窗口办理").replace("2", "网上办理").replace("3", "网上窗口均可办理").replace("4", "上门服务").replace("5", "快递申请"));
                            }else {
                            ...
                            if(StringUtil.isNotBlank(auditTaskExtension.getStr("CONSULT_URL"))) {
                                taskBasicJson.put("consulturl",  auditTaskExtension.getStr("CONSULT_URL").replace("^", "、"));
                            }else {
                                taskBasicJson.put("consulturl", "无");
                            }
                            
                            //   监督投诉方式
                            if(StringUtil.isNotBlank(auditTaskExtension.getStr("COMPLAIN_TYPE"))) {
                                taskBasicJson.put("complaintype",  auditTaskExtension.getStr("COMPLAIN_TYPE"));
                            }else {
                            ...
                            if(StringUtil.isNotBlank(complaininfo) && complaininfo.contains("version")) {
                                taskBasicJson.put("complaininfo", complaininfo.substring(97)
                               		 .replace("<TYPE>2</TYPE>", "<br>行政复议").replace("<TYPE>3</TYPE>", "<br>行政诉讼")
                            	 	 .replace("<NAME>", "<br>部门：").replace("<ADDRESS>", "<br>地址：").replace("<PHONE>", "<br>电话：")
                            	 	 .replace("</NAME>", "").replace("</ADDRESS>", "").replace("</PHONE>", "").replace("</COMPLAIN>", "").replace("<COMPLAIN>", "")
                            	 	 .replace("</COMPLAINS>", "").replace("</DATAAREA>", ""));
                            }else {
                            ...
                            String serverprojectinfo = auditTaskExtension.getStr("server_project_info");
                            if(StringUtil.isNotBlank(serverprojectinfo) && serverprojectinfo.contains("version")) {
                                taskBasicJson.put("serverprojectinfo", serverprojectinfo.substring(97)
                                		 .replace("<CHANGE_FLAG>0</CHANGE_FLAG>", "").replace("<SERVEPROJECTS>", "").replace("<SERVEPROJECT>", "")
                                	 	 .replace("<NAME>", "<br>中介服务项目名称：").replace("<BASIC_LAW>", "<br>法律依据：").replace("<RESULT_NAME>", "<br>结果名称：")
                                	 	 .replace("<REMARK>", "<br>中介服务项目名称：") .replace("</REMARK>", "").replace("</RESULT_NAME>", "")
                                	 	 .replace("</NAME>", "").replace("</BASIC_LAW>", "").replace("</SERVEPROJECT>", "")
                        ...
                        for (AuditTaskMaterial auditTaskMaterial : auditTaskMaterials) {
                            ...
                            int pagenum;
                            if (StringUtil.isBlank(auditTaskMaterial.getPage_num()) || 0 == auditTaskMaterial.getPage_num() ) {
                                pagenum = 0;
                            }else {
                            ...
                            materialJson.put("fileexplain", StringUtil.isBlank(auditTaskMaterial.getFile_explian())
                                    ? "无" : auditTaskMaterial.getFile_explian());
                            materialJson.put("publisher", StringUtil.isBlank(auditTaskMaterial.getFile_source_explain())
                            		? "无" : auditTaskMaterial.getFile_source_explain());//填报须知
                        ...
                        if(StringUtil.isNotBlank(chargeiteminfo) && chargeiteminfo.contains("version")) {
                        	dataJson.put("chargeitems", chargeiteminfo.substring(97)
                           		 .replace("<CHANGE_FLAG>0</CHANGE_FLAG>", "").replace("<CHARGEITEMS>", "").replace("<CHARGEITEM>", "").replace("TEM>", "")
                        	 	 .replace("<NAME>", "<br>收费项目名称：").replace("<STANDARD>", "<br>收费标准：").replace("<BASIS>", "<br>收费依据：")
                        	 	 .replace("<IS_REDUCTION>", "") .replace("<REDUCTION_REASON>", "") .replace("<KINDNESS>", "")
                        	 	 .replace("<SORT_ORDER>", "") .replace("0</IS_REDUCTION>", "").replace("</REDUCTION_REASON>", "").replace("</KINDNESS>", "")
                        	 	 .replace("1</SORT_ORDER>", "")
                        	 	 .replace("</NAME>", "").replace("</STANDARD>", "").replace("</BASIS>", "").replace("</CHARGEITEM>", "")
                        	 	 .replace("</CHARGEITEMS>", "").replace("</DATAAREA>", ""));
                        }else {
        ...
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if ("1".equals(isNeedQwjs)) {
                    ...
                    List<AuditTask> auditTasks = iAuditTask.getAllUsableTaskByMap(keyWord, areaCode).getResult();
        ...
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if ("1".equals(isNeedQwjs)) {
                  ...
                  //  sqlConditionUtil.setLeftJoinTable("Audit_Task_Extension b", "a.rowguid", "b.taskguid");
                   ...
                   // sqlConditionUtil.eq("IF_JZ_HALL","1");
                    ...
                    for (AuditTask auditTask : auditTasks) {
                        ...
                        taskJson.put("if_jz_hall", StringUtil.isNotBlank(if_jz_hall)?if_jz_hall:ZwfwConstant.CONSTANT_STR_ZERO);
    ...
    public String getTaskKindsByOu(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                // 2、获取事项所属的部门
                List<Record> records = iJnAppRestService.selectOuByApplyertype(applyerType, is_fugongfuchan, areaCode, taskname,isyc,null,new ArrayList<>()).getResult();
        ...
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
               ...
               // 1.3 是否支持便民服务
                ...
                // 2、获取事项所属的部门
                List<Record> records = iJnAppRestService.selectOuByApplyertypeYC(applyerType, Dao_xc_num,ISPYC,Operationscope,If_express,mpmb,CHARGE_FLAG,mashangban,wangshangban,sixshenpilb,bianminfuwu,yishenqing,qctb,ggfw,xzxk,areaCode,null,null).getResult();
                List<JSONObject> ouJsonList = new ArrayList<JSONObject>();
                // 3、获取部门图片的地址（微信端用）
                String urlRoot = WebUtil.getRequestCompleteUrl(request) + "/epointzwmhwz/css/images/oupic/";
                for (Record record : records) {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if (auditTaskFaqs != null && auditTaskFaqs.size() > 0) {
                    for (AuditTaskFaq auditTaskFaq : auditTaskFaqs) {
                        ...
                        faqJson.put("faqguid", auditTaskFaq.getRowguid());// 常见问题标识
                        faqJson.put("question", auditTaskFaq.getQuestion());// 问题
                        faqJson.put("ouname", iAuditTask.getCopyTaskByTaskId(auditTaskFaq.getTaskid(),"1").getResult().getOuname());//部门名称
                        faqJson.put("ouguid", iAuditTask.getCopyTaskByTaskId(auditTaskFaq.getTaskid(),"1").getResult().getOuguid());//部门ID
                        faqJson.put("answer", auditTaskFaq.getAnswer());// 回答
                        taskfaqList.add(faqJson);
                    }
                }
    ...
    public JSONObject handleGetTaskList(JSONObject obj, HttpServletRequest request, String function) {
       ...
       // 1.3 是否转向秒批秒办展示
        ...
        if ("1".equals(isNeedQwjs)) {
           ...
           /* if (StringUtil.isNotBlank(ztfl) && !ztfl.contains("--请选择--") && !"\"\"".equals(ztfl) && ztfl.contains("GR")) {
                ztfl = ztfl.replace("GR", "");
                sqlConditionUtil.like("b.Taskclass_forpersion", ztfl);
            }
            if (StringUtil.isNotBlank(ztfl) && !ztfl.contains("--请选择--") && !"\"\"".equals(ztfl) && ztfl.contains("FR")) {
                ztfl = ztfl.replace("FR", "");
                sqlConditionUtil.like("b.Taskclass_forcompany", ztfl);
            }*/
            ...
            if (StringUtil.isNotBlank(is_fugongfuchan) || StringUtil.isNotBlank(Dao_xc_num) || StringUtil.isNotBlank(ISPYC) || StringUtil.isNotBlank(Operationscope) 
            		|| StringUtil.isNotBlank(If_express) || StringUtil.isNotBlank(CHARGE_FLAG) || StringUtil.isNotBlank(sixshenpilb)) {
            ...
            if (StringUtil.isNotBlank(isyc) && "1".equals(isyc)) {
            	sqlConditionUtil.eq("businesstype", "1");
            }
            ...
            if (StringUtil.isNotBlank(applyerType) && ( !"1".equals(isyc) || "1".equals(qctb))) {
                sqlConditionUtil.like("applyertype", applyerType);// 申请人类型
            }
            ...
            if (StringUtil.isNotBlank(sixshenpilb) && "1".equals(sixshenpilb)) {
            	sqlConditionUtil.in("shenpilb", "'01','10','07','05','08','06','11'");;// 主题分类标识
            }
            if (StringUtil.isNotBlank(keyWord)) {
            ...
            if (StringUtil.isNotBlank(newCategory) && ZwfwConstant.CONSTANT_STR_ONE.equals(newCategory) && !"1".equals(turnmpmb)) {
                ...
                // 3.1.2.4、不属于乡镇事项的主项列表
                	...
                	fatherTaskPageData = iJnAppRestService.getGBAuditTaskPageData(sqlConditionUtil.getMap(), firstResultTask,
                ...
                // 3.1.2.6、不属于乡镇事项
                	...
                	if ("1".equals(turnmpmb)) {
                		String field = "rowguid,if_entrust,applyertype,item_id,taskname,ouname,ordernum,is_enable,ouguid,PROCESSGUID,TYPE,AREACODE,task_id,promise_day,anticipate_day,charge_flag,shenpilb,link_tel,supervise_tel,JBJMODE,BY_LAW,turn_type,transact_addr,Transact_time,turn_type";
                		fatherTaskPageData =  iAuditTask.getAuditEnableTaskPageData(field, sqlConditionUtil.getMap(), firstResultTask, Integer.parseInt(pageSizeTask), "ordernum", "desc").getResult();
                	}else {
                		 fatherTaskPageData = iHandleTask.getAuditTaskListByCondition(true, null, sqlConditionUtil.getMap(),
                                 firstResultTask, Integer.parseInt(pageSizeTask), "ordernum", "desc").getResult();
                	}
...
//                            .getResult();
    ...
    public JSONObject commonTaskJson(AuditTask auditTask, AuditOnlineRegister auditOnlineRegister, String type) {
        ...
        if (flag) {
        	auditTaskJson.put("onlinehandle", Integer.parseInt(onlineHandle)); // 事项是否可以网上申报  （事项配置在窗口，是否可以申报由事项决定）    
        }
        else {
            auditTaskJson.put("onlinehandle",  0); // 事项是否可以网上申报 （事项未配置在窗口，不可以网上申报）     
        }
        if (isAppoint(auditTask.getTask_id())) {
            auditTaskJson.put("appointment",
                    auditTaskExtension == null ? "0" : auditTaskExtension.getReservationmanagement());// 事项是否可以网上预约
    ...
    public String getTaskListByContension(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                List<AuditTask> list = zwdtService.getTaskListByContension(taskname, ouguid, Integer.parseInt(currentPageTask)*Integer.parseInt(pageSizeTask), Integer.parseInt(pageSizeTask));
                ...
                for (AuditTask task : list) {
                    ...
                    if (StringUtil.isNotBlank(task.getTransact_addr())){
                        task.set("tasklist", zwdtService.getTaskListByAddress(task.getTransact_addr(),taskname,ouguid));
                    }
                }
    ...
    public static  List<Record> syncbanliliucheng(String banliliucheng) {
        ...
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if (StringUtil.isNotBlank(isyc)) {
                	list = zwdtService.getShenpiListByAreacodeYC(areacode, ouguid, applyertype,dictid,Dao_xc_num,ISPYC,Operationscope,If_express,mpmb,CHARGE_FLAG,mashangban,wangshangban,sixshenpilb,yishenqing,isyc,qctb,null,null,null);
                }else {
                ...
                if (record != null ) {
                	  ...
                	  if (StringUtil.isNotBlank(feichangmanyis)) {
                		 ...
                		 if (task != null ) {
                    		 ...
                    		 if ("01".equals(shenpilb) || "10".equals(shenpilb) || "11".equals(shenpilb) || "07".equals(shenpilb) || "05".equals(shenpilb) || "06".equals(shenpilb) || "08".equals(shenpilb)) {
                           	  //平均评价满意度
