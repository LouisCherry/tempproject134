 ...
 */
@RestController
@RequestMapping("/jnzwdtCompanyCert")
public class JnAuditOnlineCompanyCertController
{
    /**
     * 日志
     */
    ...
    public String getCertType(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                String type = obj.getString("type");// 区分个人和企业，个人20，企业10 
                String companyId = obj.getString("companyid");//如果为空，则说明是个人
                ...
                if (auditOnlineRegister != null) {
                    ...
                    if (ZwdtConstant.ONLINEUSERTYPE_PERSON.equals(type) || StringUtil.isBlank(type)) {
                        certOwnerType = ZwdtConstant.CERTOWNERTYPE_PERSON;
                        // 1.1、获取用户信息          
                        if (auditOnlineRegister != null) {
                            ...
                            idnumber = auditOnlineIndividual.getIdnumber();
                        }
                    }
                    else {
                        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>安全性问题处理开始<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        // 2、企业类型需要获取当前登录用户是否在该企业下
                        ZwdtUserAuthValid zwdtUserAuthValid = new ZwdtUserAuthValid();
                        if (!zwdtUserAuthValid.userValid(auditOnlineRegister, companyId,
                                ZwdtConstant.USERVALID_COMPANY_PROJECT)) {
                            return JsonUtils.zwdtRestReturn("0", "UserValid企业身份验证失败！", "");
                        }
                        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>安全性问题处理结束<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        certOwnerType = ZwdtConstant.CERTOWNERTYPE_COMPANY;
                        ...
                        if (auditRsCompanyBaseinfo != null) {
                            if (StringUtil.isNotBlank(auditRsCompanyBaseinfo.getCreditcode())) {
                                ...
                                idnumber = auditRsCompanyBaseinfo.getCreditcode();
                                creditJson.put("idnumber", idnumber);
                                idList.add(creditJson);
                            }
                            if (StringUtil.isNotBlank(auditRsCompanyBaseinfo.getOrgancode())) {
                                JSONObject organjson = new JSONObject();
                                idnumber = auditRsCompanyBaseinfo.getOrgancode();
                                organjson.put("idnumber", idnumber);
                    ...
                    for (CodeItems item : codeItems) {
                        if (item.getItemValue().startsWith(ZwdtConstant.CERTOWNERTYPE_COMPANY)
                    ...
                    for (CodeItems items : personalCodeItems) {
                        ...
                        // 4.2、对list去重
                        HashSet<JSONObject> h = new HashSet<JSONObject>(idList);
                        idList.clear();
                        idList.addAll(h);
                        if (idList != null && idList.size() > 0) {
                            for (int i = 0; i < idList.size(); i++) {
                                String idNumber = idList.get(i).getString("idnumber");
                                infoList = iCertInfoExternal.selectCertInfo(certOwnerType, idNumber,
                                        items.getItemValue(), "", ZwdtConstant.MATERIALTYPE_CERT,
    ...
    public String getCertListByType(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                String type = obj.getString("type");//个人还是企业
                ...
                if (auditOnlineRegister != null) {
                    ...
                    if (idList != null && idList.size() > 0) {
                        for (int i = 0; i < idList.size(); i++) {
                            String idNumber = idList.get(i).getString("idnumber");
                            infoList = iCertInfoExternal.selectCertInfo(certOwnerType, idNumber, certType, certLevel,
                                    ZwdtConstant.MATERIALTYPE_CERT, ZwdtConstant.CERTAPPKEY, false, "",null);
                            certInfoList.addAll(infoList);
                        }
                    }
                    ...
                    List<CertInfo> certInfoCList = new ArrayList<CertInfo>();
                    List<CertInfo> infoCList = null;
                    if (idList != null && idList.size() > 0) {
                        for (int i = 0; i < idList.size(); i++) {
                            String idNumber = idList.get(i).getString("idnumber");
                            infoCList = iCertInfoExternal.selectCertByOwner(certCatalogids, certOwnerType, "", idNumber,
                                    false, ZwdtConstant.CERTAPPKEY, null);
                            certInfoCList.addAll(infoCList);
                        }
                    }
                    ...
                    //添加大数据相关的证照文件
                    List<FrameAttachInfo> attachinfos =  iJnProjectService.getFrameAttachByIdenumberTag(idnumber,"大数据电子证照");
                    if (attachinfos != null && attachinfos.size() > 0) {
                    	for (FrameAttachInfo attachinfo : attachinfos) {
                            ...
                            certJson.put("coversrc", WebUtil.getRequestCompleteUrl(request)
                                    + "/rest/jnattachcontent/readAttach?attachguid=" + attachinfo.getAttachGuid()
    ...
    public String getCertDetail(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if (auditOnlineRegister != null) {
                    ZwdtUserAuthValid zwdtUserAuthValid = new ZwdtUserAuthValid();
                    if (!zwdtUserAuthValid.companyCertUserValid(auditOnlineRegister, shareMaterialIGuid, companyId)) {
                                ...
                                // 文件上传
                                    ...
                                    if (StringUtil.isNotBlank(clientGuid)) {
                                        ...
                                        if (attachinfolist != null && attachinfolist.size() > 0) {
                                            ...
                                            String frameAttachGuid = ParamEncryptUtil.encrypt(attachinfolist.get(0).getAttachGuid(),
                                                    sysLoginId, null);
                                            dataTable.put("frameAttachGuid", frameAttachGuid);
                                        ...
                                        for (CertMetadata certMetadataSub : certMetadatas) {
                                            ...
                                            // 文件上传
                                                ...
                                                if (StringUtil.isNotBlank(clientGuid)) {
                                                    ...
                                                    if (attachinfolist == null || attachinfolist.size() == 0) {
                                                        List<JSONObject> attachList = iCertAttachExternal
                                                                .getAttachList(clientGuid, ZwdtConstant.CERTAPPKEY);
                                                        for (JSONObject json : attachList) {
                                                            //独立证照库情况下，将附件存储到本地附件库中。本地模式下，实现附件复制
                                                            AttachUtil.saveFileInputStream(UUID.randomUUID().toString(),
                                                                    clientGuid, json.getString("attachname"),
                                                                    json.getString("contentype"),
                                                                    json.getString("attachstorageguid"),
                                                                    json.getLong("size"), iCertAttachExternal.getAttach(
                                                                            json.getString("attachguid"), ""),
                                                                    "", "");
                                                        }
                                                    }
                                                    attachinfolist = iAttachService.getAttachInfoListByGuid(clientGuid);
                ...
                if (auditOnlineRegister != null) {
                    ...
                    if (ZwdtConstant.ONLINEUSERTYPE_PERSON.equals(type) || StringUtil.isBlank(type)) {
                        ...
                        auditOnlineIndividual = iAuditOnlineIndividual
                                .getIndividualByAccountGuid(auditOnlineRegister.getAccountguid()).getResult();
                        idnumber = auditOnlineIndividual.getIdnumber();
                        certOwnerName = auditOnlineIndividual.getClientname();
                        certOwnerCertType = ZwdtConstant.CERTOWNERCERTTYPE_ID;
                    }
                    else {
                        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>安全性问题处理开始<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                        // 2、企业类型需要获取当前登录用户是否在该企业下
                        ...
                        if (auditRsCompanyBaseinfo != null) {
                            if (StringUtil.isNotBlank(auditRsCompanyBaseinfo.getCreditcode())) {
                                idnumber = auditRsCompanyBaseinfo.getCreditcode();
                                certOwnerCertType = ZwfwConstant.CERT_TYPE_TYSHXYDM;
                            }
                            else if (StringUtil.isNotBlank(auditRsCompanyBaseinfo.getOrgancode())) {
                                idnumber = auditRsCompanyBaseinfo.getOrgancode();
                                certOwnerCertType = ZwfwConstant.CERT_TYPE_ZZJGDMZ;
                            }
                            if (StringUtil.isNotBlank(auditRsCompanyBaseinfo.getOrganlegal())) {
                                certOwnerName = auditRsCompanyBaseinfo.getOrganlegal();
                            }
                        }
                    }
                    CertInfo certInfo = iCertInfoExternal.getCertInfoByRowguid(shareMaterialIGuid);
                    if (certInfo != null) {
                        ...
                        newCertInfo.setCertownertype(certOwnerType); //持有人类型
                        newCertInfo.setCertownerno(idnumber); //持有人证件号码
                        newCertInfo.setCertownername(certOwnerName); //持有人姓名
                        ...
                        newCertInfo.setCertownercerttype(certOwnerCertType);//持有人证件类型
                ...
                if (StringUtil.isNotBlank(shareMaterialIGuid)) {
                    ...
                    if (auditOnlineRegister != null) {
                        ...
                        if (StringUtil.isBlank(companyId)) {
                            ...
                            if (!zwdtUserAuthValid.companyCertUserValid(auditOnlineRegister, shareMaterialIGuid,
                                    companyId)) {
                                return JsonUtils.zwdtRestReturn("0", "UserValid身份验证失败！", "");
                            }
                        }
        ...
        try {
            ...
            if (ZwdtConstant.SysValidateData.equals(token)) {
                ...
                if (auditOnlineRegister != null) {
                    ...
                    // 3、获取用户拥有的所有证照实例信息
                    List<CertInfo> certInfoList = iCertInfoExternal.selectCertInfo(certOwnerType, idnumber, "", "",
                            ZwdtConstant.MATERIALTYPE_CERT, ZwdtConstant.CERTAPPKEY, false, "",null);
