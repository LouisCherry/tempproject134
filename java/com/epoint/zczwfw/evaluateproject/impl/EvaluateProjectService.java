package com.epoint.zczwfw.evaluateproject.impl;

import java.util.List;
import java.util.Map;

import com.epoint.common.util.SQLManageUtil;
import com.epoint.common.util.StringUtil;
import com.epoint.core.dao.CommonDao;
import com.epoint.core.dao.ICommonDao;
import com.epoint.core.grammar.Record;
import com.epoint.zczwfw.evaluateproject.api.entity.EvaluateProject;

/**
 * 评价办件信息表对应的后台service
 * 
 * @author yrchan
 * @version [版本号, 2022-04-11 15:52:28]
 */
public class EvaluateProjectService
{
    /**
     * 数据增删改查组件
     */
    protected ICommonDao baseDao;

    public EvaluateProjectService() {
        baseDao = CommonDao.getInstance();
    }

    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(EvaluateProject record) {
        return baseDao.insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public <T extends Record> int deleteByGuid(String guid) {
        T t = baseDao.find(EvaluateProject.class, guid);
        return baseDao.delete(t);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(EvaluateProject record) {
        return baseDao.update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public EvaluateProject find(Object primaryKey) {
        return baseDao.find(EvaluateProject.class, primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public EvaluateProject find(String sql, Object... args) {
        return baseDao.find(sql, EvaluateProject.class, args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<EvaluateProject> findList(String sql, Object... args) {
        return baseDao.findList(sql, EvaluateProject.class, args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<EvaluateProject> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return baseDao.findList(sql, pageNumber, pageSize, EvaluateProject.class, args);
    }

    /**
     * 查询数量
     * 
     * @param sql
     *            执行语句
     * @param args
     *            参数
     * @return Integer
     */
    public Integer countEvaluateProject(String sql, Object... args) {
        return baseDao.queryInt(sql, args);
    }

    /**
     * 
     * 判断办结时间为同一天且同一个手机号的评价办件信息是否存在
     * 
     * @param link_phone
     * @param handleDate
     * @return
     */
    public boolean isExistPhoneAndHandleDate(String link_phone, String handleDate) {
        String sql = "select count(1) from evaluate_project where link_phone = ? and handle_date >= ? and handle_date <= ? ";
        int count = baseDao.queryInt(sql, link_phone, handleDate + " 00:00:00", handleDate + " 23:59:59");
        return count > 0;
    }

    /**
     * 
     * 通过条件（时间）获取最大编号
     * 
     * @param map
     *            条件
     * @return
     */
    public String getMaxNoByCondition(Map<String, String> map) {
        StringBuilder sql = new StringBuilder();
        sql.append("select max(project_no) from evaluate_project where 1=1 ");
        String searchCondition = new SQLManageUtil().buildPatchSql(map);
        if (StringUtil.isNotBlank(searchCondition)) {
            sql.append(searchCondition);
        }
        return baseDao.queryString(sql.toString());
    }
}
