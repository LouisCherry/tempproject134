package com.epoint.myapplication.ans8583;


import com.epoint.socket.SocketClient;
import com.epoint.socket.iso8583.Iso8583Utils;

import java.util.Arrays;

import static com.epoint.myapplication.utils.DesUtil.*;
...
public class My8583Ans extends Easy8583Ans {
    ...
    //需要持久化存储这些参数，每次使用时加载
    private static String manNum  = "fgscssd00000001"; //商户号
    private static String posNum  = "10000002"; //终端号
    private static String mainKey = "44444444444444444444444444444444"; //主秘钥
    private static String TPDU    = "6000000000";
    ...
    public My8583Ans(){
        ...
        pack.head = hexStringToBytes("602200000000");
        upBinNumber = "";
        cardSnNumber = "";
        cardValiteDate="";
        field35Data = "";
    ...
    public int ans8583Qrcode(byte[] rxbuf,int rxlen){
        ...
        //应答码判断
        if((fieldsRecv[38].data[0] != 0x30)||(fieldsRecv[38].data[1] != 0x30)){
            //system.out.println("应答码不正确！");
            //system.out.println(String.format("应答码:%02x%02x",fieldsRecv[38].data[0],fieldsRecv[38].data[1]));
            return 3;
        }
        //跟踪号比较
        if(!Arrays.equals(fieldsSend[10].data,fieldsRecv[10].data)){
    ...
    public static void main(String[] args) {
        ...
        String recvstr = SocketClient.send(send);
