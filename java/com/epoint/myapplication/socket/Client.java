package com.epoint.myapplication.socket;
	...
	private void creatSocket() {
		if (!this.socket.isConnected() || this.socket.isClosed()) {
			new Thread() {
				public void run() {
					try {
						//system.out.println( ip + ":" + port + "准备连接");
						// 开始连接
...
//						socket.connect(new InetSocketAddress(ip, port), 6000);
						...
						e.printStackTrace();
						//system.out.println( ip + ":" + port + "服务器连接不上");
					}
				}
			}.start();
		}
	...
	public void close() {
		try {
			// 关闭套接字连接
			this.socket.close();
			//system.out.println(ip + ":" + port + "已断开连接");
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void receiveMsg() {
		try {
			...
			//system.out.println( "socket连接断开了，测试-1");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
