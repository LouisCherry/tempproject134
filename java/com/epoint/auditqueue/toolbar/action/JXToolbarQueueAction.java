...
@RestController("jxtoolbarqueueaction")
@Scope("request")
public class JXToolbarQueueAction extends BaseController {
		...
		if (!QueueConstant.Window_WorkStatus_Pause.equals(getHidStatus())) {
			...
			addCallbackParam("QNO",
					handletoolbarservice.startQueue(zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
							zwfwuserSession.getCenterGuid(), userSession.getUserGuid(), true).getResult());
			GetWaitNum();
	...
	public void nextclick(String QNO) {
		addCallbackParam("QNO",
		        jxhandletoolbarservice.nextQueue(QNO, zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
						zwfwuserSession.getCenterGuid(), userSession.getUserGuid(), true).getResult());
		GetWaitNum();
	}
