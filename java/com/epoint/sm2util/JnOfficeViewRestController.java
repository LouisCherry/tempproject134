...
@RestController
@RequestMapping("/jnofficeview")
public class JnOfficeViewRestController
{
    ...
    public String getSignFurl(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            ...
            // officeweb服务器地址
            // String officeweb365url =
            // handleConfigservice.getFrameConfig("OFFICEWEB365_URL",
            // centerguid).getResult();
            String rootpath = QueueCommonUtil.getUrlPath(request.getRequestURL().toString())
                    + "/rest/auditattach/readAttach?attachguid=" + exampleattachguid;
            if (exampleattachguid.indexOf("http") != -1) {
                rootpath = exampleattachguid;
            }
    ...
    @RequestMapping(value = "/newgetsignfurl", method = RequestMethod.POST)
    public String newgetsignfurl(@RequestBody String params, @Context HttpServletRequest request) {
        try {
            JSONObject json = JSON.parseObject(params);
            JsonUtils.checkUserAuth(json.getString("token"));
            JSONObject obj = (JSONObject) json.get("params");
            ...
            // centerguid).getResult();
            String rootpath = QueueCommonUtil.getUrlPath(request.getRequestURL().toString())
                    + "/rest/jnattachcontent/readAttach?attachguid=" + exampleattachguid;
            if (exampleattachguid.indexOf("http") != -1) {
                rootpath = exampleattachguid;
            }
            // 加密key
