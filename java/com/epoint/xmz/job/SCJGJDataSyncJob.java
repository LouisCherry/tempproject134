...
public class SCJGJDataSyncJob implements Job {
    private Logger log = Logger.getLogger(MethodHandles.lookup().lookupClass());
    private IJnService jnService = ContainerFactory.getContainInfo().getComponent(IJnService.class);
    @Override
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        try {
            log.info("==========开始执行SCJGJDataSyncJob数据同步推送服务==========");
            EpointFrameDsManager.begin(null);

            tbApplyBaseInfo();

            EpointFrameDsManager.commit();
            log.info("==========执行SCJGJDataSyncJob数据同步推送服务成功！！！==========");
        } catch (Exception e) {
            e.printStackTrace();
            EpointFrameDsManager.rollback();
            log.info("==========执行SCJGJDataSyncJob数据同步推送服务失败！！！==========");
        } finally {
            EpointFrameDsManager.close();
            log.info("==========执行SCJGJDataSyncJob数据同步推送服务结束==========");
        }
    }
