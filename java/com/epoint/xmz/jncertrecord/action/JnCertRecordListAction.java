...
/**
 * 证照调用次数统计表list页面对应的后台
 * 
 * @author 1
 * @version [版本号, 2022-08-22 16:53:37]
 */
@RestController("jncertrecordlistaction")
@Scope("request")
public class JnCertRecordListAction  extends BaseController
{
	@Autowired
     ...
     * 证照调用次数统计表实体对象
     */
    private JnCertRecord dataBean;
  
    /**
     * 表格控件model
     */
    private DataGridModel<JnCertRecord> model;
    ...
    public DataGridModel<JnCertRecord> getDataGridData() {
        // 获得表格对象
        if (model == null) {
            model = new DataGridModel<JnCertRecord>()
            {

                @Override
                public List<JnCertRecord> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    String sql = "select areacode,count(1) as total from jn_cert_record where 1=1 ";
    ...
    public ExportModel getExportModel() {
        if (exportModel == null) {
            exportModel = new ExportModel("areacode,recordtotal", "辖区代码,调用次数");
        }
        return exportModel;
    }
