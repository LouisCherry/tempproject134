...
public class JnSheBaoController {
	/**
	 * 日志
	 */
	 ...
	 * @return
	 */
	@RequestMapping(value = "/techniqueCertQuery", method = RequestMethod.POST)
	public String techniqueCertQuery(@RequestBody String params) {
		try {
			log.info("=======开始调用techniqueCertQuery接口=======");
			// 1、接口的入参转化为JSON对象
			...
			String TenderNum = jsonObject.getString("xm");
			String TenderProjectCode = jsonObject.getString("sfzh");
			String TenderCorpName = jsonObject.getString("czzsbh");
			...
			if (StringUtil.isNotBlank(result)) {
				...
				if ("true".equals(jsonresult.getString("success"))) {
					...
					log.info("=======结束调用techniqueCertQuery接口=======");
			...
			e.printStackTrace();
			log.info("=======techniqueCertQuery接口参数：params【" + params + "】=======");
			log.info("=======techniqueCertQuery异常信息：" + e.getMessage() + "=======");
