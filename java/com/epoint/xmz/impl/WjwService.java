...
public class WjwService
{
     ...
     * 前置库数据源
     */
//    private static String jnzjxtdjurl = ConfigUtil.getConfigValue("datasyncjdbc", "wjwjurl");
//    private static String jnzjxtdjusername = ConfigUtil.getConfigValue("datasyncjdbc", "wjwusername");
//    private static String jnzjxtdjpassword = ConfigUtil.getConfigValue("datasyncjdbc", "wjwpassword");
...
//        jnzjxtdjDao = CommonDao.getInstance(jnzjxtdjDataSourceConfig);
    ...
    public List<Record> getSCJGJApplyBaseInfo(Date beginOfDate,Date endOfDate){
        String sql = " select * from inf_apply_baseinfo_r where ifnull(sync_sign,0) = 0 limit 200";
        List<Record> list = jnzjxtdjDao.findList(sql, Record.class);
        return list;
    }
    ...
    public void upApplyBaseInfoSign(String rowguid,String Status){
        String sql = " update inf_apply_baseinfo_r set SYNC_SIGN =? where RowGuid =? ";
        jnzjxtdjDao.execute(sql,Status,rowguid);
