...
import com.epoint.xmz.thirdreporteddata.basic.spglv3.service.SpglZrztxxbV3Service;
...
public class SpglZrztxxbV3ServiceImpl implements ISpglZrztxxbV3Service
{
    ...
    public int insert(SpglZrztxxbV3 record) {
        ...
        //法人证件类型为空情况处理
        if (StringUtil.isNotBlank(record.getFrzjlx())) {
            record.setFrzjlx(1);
        }

        //负责人人证件类型为空情况处理
        if (StringUtil.isNotBlank(record.getFzrzjlx())) {
            record.setFzrzjlx(1);
        }

        // 法人身份证号码为空情况处理
        if (StringUtil.isNotBlank(record.getFrzjhm())) {
            AuditRsItemBaseinfo auditRsItemBaseinfo = iAuditRsItemBaseinfo.getAuditRsItemBaseinfoByItemcode(record.getXmdm()).getResult();
            if (auditRsItemBaseinfo != null) {
                record.setFrzjhm(auditRsItemBaseinfo.getLegalpersonicardnum());
            }
        }
    ...
    @Override
    public AuditCommonResult<PageData<SpglZrztxxbV3>> getAllByPage(Map<String, String> conditionMap, Integer first,
    ...
    public List<ParticipantsInfo> getParticipantsInfoListBySubappguid(String subappguid) {
        return new SpglZrztxxbV3Service().getParticipantsInfoListBySubappguid(subappguid);
    }
}
