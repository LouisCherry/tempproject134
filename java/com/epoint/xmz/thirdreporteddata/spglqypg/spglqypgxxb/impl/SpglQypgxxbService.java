package com.epoint.xmz.thirdreporteddata.spglqypg.spglqypgxxb.impl;
...
public class SpglQypgxxbService {
    ...
    public PageData<SpglQypgxxb> getNotAssociationPageData(String itemguid, int firstResult, int maxResults,
        ...
        String sql = "select * from spgl_qypgxxb_edit where rowguid not in (select qypgguid from spgl_qypgxxb_edit_r where pre_itemguid = ?) ";
        String sql2 = "select count(1) from spgl_qypgxxb_edit where rowguid not in (select qypgguid from spgl_qypgxxb_edit_r where pre_itemguid = ?) ";
        ...
        List<SpglQypgxxb> list = baseDao.findList(sql, firstResult, maxResults, SpglQypgxxb.class, itemguid);
        PageData<SpglQypgxxb> pageData = new PageData<SpglQypgxxb>();
        pageData.setList(list);
        pageData.setRowCount(baseDao.queryInt(sql2, itemguid));
        return pageData;
    }

    public List<SpglQypgxxb> getSpglQypgxxbListByitemguid(String itemguid) {
        String sql = "select e.* from spgl_qypgxxb_edit e,spgl_qypgxxb_edit_r er where er.pre_itemguid=? and e.RowGuid=er.qypgguid";
        return baseDao.findList(sql, SpglQypgxxb.class, itemguid);
    }
