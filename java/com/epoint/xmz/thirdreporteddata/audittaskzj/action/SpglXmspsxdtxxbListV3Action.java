 ...
 */
@RestController("spglxmspsxdtxxblistv3action")
@Scope("request")
public class SpglXmspsxdtxxbListV3Action extends BaseController
{
    /**
     *
     */
     ...
     * 实体对象
     */
    private SpglXmdtxxbV3 dataBean;

    /**
     * 表格控件model
     */
    private DataGridModel<SpglXmdtxxbV3> model;
    ...
    public void pageLoad() {
        ...
        if (dataBean == null) {
            dataBean = new SpglXmdtxxbV3();
        }
    }

    public DataGridModel<SpglXmdtxxbV3> getDataGridData() {
        // 获得表格对象
        if (model == null) {
            model = new DataGridModel<SpglXmdtxxbV3>()
            {
                ...
                public List<SpglXmdtxxbV3> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if (StringUtil.isNotBlank(dataBean.getDtmc())) {
                        sUtil.like("dtmc", dataBean.getDtmc());
                    }
                    ...
                    PageData<SpglXmdtxxbV3> pageData = service.getAllByPage(sUtil.getMap(), first, pageSize, sortField,
                            sortOrder).getResult();
                    this.setRowCount(pageData.getRowCount());
                    List<SpglXmdtxxbV3> list = pageData.getList();
                    if (ValidateUtil.isNotBlankCollection(list)) {
                        for (SpglXmdtxxbV3 bean : list) {
                            // 本地校验失败
                            if (bean.getSjsczt() == -1) {
