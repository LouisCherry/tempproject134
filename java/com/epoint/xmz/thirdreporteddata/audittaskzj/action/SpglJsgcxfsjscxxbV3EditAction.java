...
/**
 * 建设工程消防设计审查信息表修改页面对应的后台
 *
 * @author Epoint
 * @version [版本号, 2023-11-01 16:15:31]
 */
@RightRelation(com.epoint.xmz.thirdreporteddata.audittaskzj.action.SpglJsgcxfsjscxxbV3ListAction.class)
@RestController("spgljsgcxfsjscxxbv3editaction")
@Scope("request")
public class SpglJsgcxfsjscxxbV3EditAction extends BaseController {
     ...
     * 建设工程消防设计审查信息表实体对象
     */
    private SpglJsgcxfsjscxxbV3 dataBean = null;

    /**
     * 建筑保温材料类别下拉列表model
     */
    private List<SelectItem> bwcllbModel = null;
    ...
    public void save() {
        ...
        try {
            ...
            if (StringUtil.isNotBlank(dataBean.getRowguid())) {
                SpglJsgcxfsjscxxbV3 oldDataBean = service.findDominByCondition(xzqhdm, gcdm, spsxslbm);
