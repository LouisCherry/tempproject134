...
@RestController("spglsbsjlcjdsxctv3action")
@Scope("request")
public class SpglSbsjlcjdsxctV3Action extends BaseController
{

    private static final long serialVersionUID = 8796953926146877276L;

    transient Logger log = LogUtil.getLog(SpglSbsjlcjdsxctV3Action.class);

    /**
     * 流程信息实体对象
     */
    private SpglSpsxjbxxb dataBean;
    ...
    public void reSyncBusiness() {
        ...
        try {
            ...
            SpglSpsxjbxxb oldDataBean = new SpglSpsxjbxxb();
            Spglsplcjdsxxxb find = iSpglsplcjdsxxxb.find(rowguid);
            if (find != null) {
                SqlConditionUtil sqlConditionUtil = new SqlConditionUtil();
                sqlConditionUtil.eq("xzqhdm", find.getXzqhdm());
    ...
    public SpglSpsxjbxxb getDataBean() {
        if (dataBean == null) {
            dataBean = new SpglSpsxjbxxb();
        }
        return dataBean;
    }

    public void setDataBean(SpglSpsxjbxxb dataBean) {
