...
@RestController("spglsbsjlccllistv3action")
@Scope("request")
public class SpglSbsjlcclListV3Action extends BaseController
{

    private static final long serialVersionUID = 8796953926146877276L;

    transient Logger log = LogUtil.getLog(SpglSbsjlcclListV3Action.class);
    ...
    @Autowired
    private ISpglsplcjdsxxxb ipSpglsplcjdsxxxb;
        ...
        if (model == null) {
            model = new DataGridModel<Spglsplcxxb>()
            {
                ...
                public List<Spglsplcxxb> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    PageData<Spglsplcxxb> pageData = iSpglsplcxxb.getAllByPage(sql.getMap(), first, pageSize, sortField,
                            sortOrder).getResult();
                    ...
                    if (ValidateUtil.isNotBlankCollection(lists)) {
                        for (Spglsplcxxb lcxxb : lists) {
                            Double maxSplcbbh = iSpglsplcxxb.getMaxSplcbbh(lcxxb.get("splcbm"));
                            if (maxSplcbbh.toString().equals(lcxxb.getSplcbbh().toString())) {
                                ...
                                Integer subCount = iSpglsplcxxb.getCountByCondition(sqlc.getMap()).getResult();
                                if (subCount > 0) {
                ...
                public List<Spglsplcxxb> fetchChildrenData(JSONObject t) {
                    ...
                    sql.nq("IFNULL(sync,0)", "-1");
                    sql.setOrderDesc("splcbbh");
                    lists = iSpglsplcxxb.getListByCondition(sql.getMap()).getResult();
                    if (ValidateUtil.isNotBlankCollection(lists)) {
                        for (Spglsplcxxb Spglsplcxxb : lists) {
                            Spglsplcxxb.put("id", Spglsplcxxb.getRowguid());
                            Spglsplcxxb.put("pid", t.get("pid"));
                            Spglsplcxxb.put("checked", false);
                            Spglsplcxxb.put("expanded", false);
                            Spglsplcxxb.put("isLeaf", true);
                            // 本地校验失败
                            if (Spglsplcxxb.getSjsczt() == -1) {
                                Spglsplcxxb.put("sjscztText", "本地校验失败");
                            }
                            else {
                                Spglsplcxxb.put("sjscztText",
                                        codeItemsService.getItemTextByCodeName("国标_数据上传状态",
                                                Spglsplcxxb.getSjsczt().toString()));
                            }
                            Spglsplcxxb.put("operatedateText",
                                    EpointDateUtil.convertDate2String(Spglsplcxxb.getOperatedate(),
                                            "yyyy-MM-dd HH:mm:ss"));
                        }
                    }
                    return lists;
    ...
    public void SyncBusiness(String rowguid) {
        ...
        try {
            ...
            spglsplcxxb.setXzqhdm(ZwfwUserSession.getInstance().getAreaCode());
            ...
            List<String> listsptaskbxjd = iauditsptask.getBasetaskBySplclxAndPhaseid(
                    String.valueOf(business.getSplclx()), "5").getResult();
            if (EpointCollectionUtils.isNotEmpty(phases)) {
                for (AuditSpPhase phase : phases) {
                    ...
                    spglspjdxxb.setXzqhdm(ZwfwUserSession.getInstance().getAreaCode());
                    ...
                    // 修改为推送所有辖区这个，同一主题类型，统一阶段id下的所有
                    List<String> baseTaskguidList = iauditsptask.getBasetaskBySplclxAndPhaseid(
                            String.valueOf(business.getSplclx()), phase.getPhaseId()).getResult();
                    // 去除空串
                    baseTaskguidList = baseTaskguidList.stream().filter(a -> StringUtil.isNotBlank(a))
                            .collect(Collectors.toList());

                    // 去除所有的主题的并行阶段事项
                    if (!"5".equals(phase.getPhaseId())) {
                        baseTaskguidList = baseTaskguidList.stream().filter(a -> !listsptaskbxjd.contains(a))
                                .collect(Collectors.toList());
                    }

                    for (String baseTaskguid : baseTaskguidList) {
                        SqlConditionUtil sqlc = new SqlConditionUtil();
                        sqlc.eq("basetaskguid", baseTaskguid);
                        ...
                        for (AuditSpBasetaskR auditSpBasetaskR : taskrlist) {
                            ...
                            spglsplcjdsxxxb.setDfsjzj(baseTaskguid);
                            spglsplcjdsxxxb.setXzqhdm(ZwfwUserSession.getInstance().getAreaCode());
                            ...
                            ipSpglsplcjdsxxxb.insert(spglsplcjdsxxxb);
                            ...
                            List<AuditSpBasetask> basetasks = iauditspbasetask.getAuditSpBasetasksBytaskid(
                                    audittask.getTask_id(),businesstype);
