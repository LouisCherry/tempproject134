 ...
 * @作者 shibin
 */
@RestController("jnauditprojectmonitorhandleaction")
@Scope("request")
public class JnAuditProjectMonitorHandleAction extends BaseController {
    ...
    public void pageLoad() {
        ...
        //String guid = dao.queryString(sql, dataBean.getRowguid());
        String guid = jnAuditJianGuanService.getFileclientguidFromMonitor(dataBean.getRowguid());
        // 获取附件信息
        if (StringUtil.isNotBlank(guid)) {
            List<FrameAttachInfo> frameAttachInfoList = attachService.getAttachInfoListByGuid(guid);
        ...
        if (model == null) {
            model = new DataGridModel<AuditProjectMonitorReply>() {
                ...
                public List<AuditProjectMonitorReply> fetchData(int first, int pageSize, String sortField,
                    ...
                    List<AuditProjectMonitorReply> monitorReplies = dao.findList(sql, AuditProjectMonitorReply.class,
                            dataBean.getNo());*/
                    List<AuditProjectMonitorReply> monitorReplies = jnAuditJianGuanService
                            .getMonitorReplyByMonitorNo(dataBean.getNo());
    ...
    public void monitorBack() {
        ...
        projectMonitorReply.setReply_opinion(this.replyOpinion);
        ...
        if ("1".equals(this.isReply)) {
            ...
            if (frameRole != null) {
                ...
                if (frameUserRoleRelations != null && frameUserRoleRelations.size() > 0) {
                    Iterator<FrameUserRoleRelation> var7 = frameUserRoleRelations.iterator();

                    while (var7.hasNext()) {
                        FrameUserRoleRelation frameUserRoleRelation = var7.next();
    ...
    public String getFileClientGuid() {
        return fileClientGuid;
    }

    public void setFileClientGuid(String fileClientGuid) {
        this.fileClientGuid = fileClientGuid;
    }
