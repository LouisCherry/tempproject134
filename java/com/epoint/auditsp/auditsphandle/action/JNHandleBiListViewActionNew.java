...
/**
 * 申报查询列表页面对应的后台
 * 2023.8.28 由于原个性化类弃用,因此重新个性化了产品源码
 *
 * @author Administrator
 */
@RestController("jnhandlebilistviewactionnew")
@Scope("request")
public class JNHandleBiListViewActionNew extends BaseController {
    /**
     *
     */
            ...
            model = new DataGridModel<AuditSpInstance>() {
                ...
                public List<AuditSpInstance> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if ("370811".equals(areacode) || "370891".equals(areacode)) {
                        sql.in("areacode", "'" + areacode + "','370800'");
                    } else {
                    ...
                    for (AuditSpInstance auditSpInstance : list) {
                        ...
                        // 判断是否存在子申报数据
                        sqlc.eq("biguid", auditSpInstance.getRowguid());
                        sqlc.nq("status", "-1");
                        Integer count = subappservice.getSubappCountByCoundtion(sqlc.getMap()).getResult();
                        auditSpInstance.put("isLeaf", true);
                ...
                public List<AuditSpInstance> fetchChildrenData(JSONObject t) {
                    ...
                    for (AuditSpISubapp auditSpISubapp : list) {
                        ...
                        // 查询阶段
                        AuditSpPhase sp = spphaseservice.getAuditSpPhaseByRowguid(auditSpISubapp.getPhaseguid())
                                .getResult();
                        ...
                        if (sp != null) {
                            spi.put("phasename", sp.getPhasename());
                        }
                        listi.add(spi);
                    }
                    return listi;
    ...
    public void getFlowData() {
        ...
        AuditSpBusiness auditSpBusiness = spBusinessService.getAuditSpBusinessByRowguid(instance.getBusinessguid())
                .getResult();
        AuditRsItemBaseinfo auditRsItemBaseinfo = rsItemBaseinfoService
        ...
        Integer days = iauditorgaworkingday.GetWorkingDays_Between_From_To(
                ZwfwUserSession.getInstance().getCenterGuid(), instance.getCreatedate(), new Date()).getResult();
        ...
        for (AuditSpPhase auditSpPhase : listphase) {
            ...
            for (AuditSpISubapp auditSpISubapp : subapp) {
                if (auditSpISubapp.getSpendminutes() != null) {
                    times += auditSpISubapp.getSpendminutes();
                } else {
                    AuditProjectSparetime spareitem = iauditprojectsparetime
                            .getSparetimeByProjectGuid(auditSpISubapp.getRowguid()).getResult();
                    if (spareitem != null) {
                        times += spareitem.getSpendminutes();
                    }
                }
            ...
            phaseobj.put("enddate",
                    (isend && endDate != null) ? EpointDateUtil.convertDate2String(endDate, "y年M月d日") : "未结束");
        ...
        for (AuditSpITask auditspitask : spitasklist) {
            if (StringUtil.isNotBlank(auditspitask.getProjectguid())) {
                // 查询办件
                AuditProject ap = projectservice.getAuditProjectByRowGuid(
        ...
        for (AuditProject auditProject : listproject) {
            ...
            List<AuditSpBasetaskR> listtastr = iauditspbasetaskr.getAuditSpBasetaskrByCondition(sqlc.getMap())
                    .getResult();
            if (listtastr.isEmpty()) {
                continue;
            }
            if (taskmap.get(listtastr.get(0).getBasetaskguid()) == null) {
                ...
                projectname.append(auditProject.getProjectname()).append(" (")
                        .append(ouservice.getOuByOuGuid(auditProject.getOuguid()).getOuname()).append(")");
                // 即办件不计时
                if (auditProject.getTasktype() == Integer.parseInt(ZwfwConstant.ITEMTYPE_JBJ)) {
                    ...
                    if (auditProject.getBanjiedate() != null) {
                        int daynum = iauditorgaworkingday
                                .GetWorkingDays_Between_From_To(auditProject.getCenterguid(), new Date(), new Date())
                                .getResult();
                        projecttime.append(sdf.format(auditProject.getOperatedate())).append("-")
                                .append(sdf.format(auditProject.getBanjiedate())).append(" 用时").append(daynum)
                                .append("工作日");
                    } else {
        ...
        for (AuditSpISubapp subapp : listsub) {
            ...
            for (AuditSpITask auditspitask : spitasklist) {
                if (StringUtil.isNotBlank(auditspitask.getProjectguid())) {
                    // 查询办件
                    AuditProject ap = projectservice.getAuditProjectByRowGuid(
                            "rowguid,task_id,status,projectname,ouname,banjiedate,operatedate,ouguid,tasktype,centerguid",
                            auditspitask.getProjectguid(), auditspitask.getAreacode()).getResult();
                    if (ap != null) {
                        listproject.add(ap);
                    }
            ...
            AuditRsItemBaseinfo rsitem = rsItemBaseinfoService.getAuditRsItemBaseinfoByRowguid(subapp.getYewuguid())
                    .getResult();
            ...
            for (AuditProject auditProject : listproject) {
                ...
                if (auditProject.getTasktype() == Integer.parseInt(ZwfwConstant.ITEMTYPE_JBJ)) {
                    ...
                    if (auditProject.getBanjiedate() != null) {
                        int daynum = iauditorgaworkingday.GetWorkingDays_Between_From_To(auditProject.getCenterguid(),
                                auditProject.getOperatedate(), auditProject.getBanjiedate()).getResult();
                        if (daynum == 0) {
                            daynum = 1;
                        }
                        projecttime.append(sdf.format(auditProject.getOperatedate())).append("-")
