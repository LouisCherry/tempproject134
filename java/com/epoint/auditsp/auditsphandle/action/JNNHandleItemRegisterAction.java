 ...
 */
@RestController("jnnhandleitemregisteraction")
@Scope("request")
public class JNNHandleItemRegisterAction extends BaseController {
    /**
     *
     */
    ...
    @Autowired
    private IJNauditRsItemBaseinfoservice ijnAuditRsItemBaseinfo;
    ...
    public void pageLoad() {
        ...
        dataBean.set("SFXXGC", ZwfwConstant.CONSTANT_INT_ZERO);
        /*3.0新增逻辑结束*/
        dataBean.setIsimprovement(ZwfwConstant.CONSTANT_STR_ZERO);
        dataBean.setTdsfdsjfa(ZwfwConstant.CONSTANT_INT_ZERO);
        dataBean.setSfwcqypg(1);
        // 是否为社会投资简易低风险工程（厂房、仓库）项目
        boolean isshtzdfx = false;
        String jydfxname = configservice.getFrameConfigValue("SP_JYDFX_NAME");
        if (bussiness != null) {
            dataBean.setItemtype(String.valueOf(bussiness.getSplclx()));
            // 如果是建议低风险投资项目
            if (jydfxname.equals(bussiness.getBusinessname())) {
                isshtzdfx = true;
            }
        }
        ...
        if (StringUtil.isNotBlank(itemGuid) || StringUtil.isNotBlank(getViewData("itemGuid"))) {
            ...
            if (dataBean != null && ZwfwConstant.CONSTANT_STR_ONE.equals(dataBean.getDraft())) {
                if (StringUtil.isNotBlank(dataBean.getParentid())) {
                    AuditRsItemBaseinfo item = iAuditRsItemBaseinfo
                            .getAuditRsItemBaseinfoByRowguid(dataBean.getParentid()).getResult();
                    if (item != null) {
            ...
            if (dataBean != null) {
                String subcode = ijnAuditRsItemBaseinfo.findMaxItemCodeByParentId(dataBean.getRowguid());
                if (StringUtil.isNotBlank(subcode)) {
                    addCallbackParam("newsubcode", dataBean.getItemcode() + "-" + subcode);
                } else {
                    addCallbackParam("newsubcode", dataBean.getItemcode() + "-01");
                }
        ...
        if (auditSpPhase != null) {
            if (isshtzdfx && "3".equals(auditSpPhase.getPhaseId())) {
                addCallbackParam("isshowdfxview", "1");
            }
        ...
        if (viewgridmodel == null) {
            viewgridmodel = new DataGridModel<AuditDggtView>() {
                ...
                public List<AuditDggtView> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    PageData<AuditDggtView> pagadata = iauditDggtView
                            .selectAuditDggtViewByPage(sqlc.getMap(), first, pageSize, sortField, sortOrder)
                            .getResult();
                    if (pagadata != null) {
        ...
        if (qypgModel == null) {
            qypgModel = new DataGridModel<SpglQypgxxb>() {
                ...
                public List<SpglQypgxxb> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    sql.eq("b.pre_itemguid", getViewData("itemGuid"));
