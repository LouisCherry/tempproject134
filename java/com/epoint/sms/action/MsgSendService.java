...
public class MsgSendService
{
     ...
     * @see [类、类#方法、类#成员]
     */
    //这边只市级事项办理时发短信，事项事项插入短信时候，字段BeiZhu插入了事项所属辖区编码
    public List<MessagesCenter> selectMessageCenters() {
        String sql = "select * from Messages_Center where sendmode =1 and (content is not null or content <> '')"
                + " and (issend = '' or issend is null or issend=0) and (messagetarget <> '' and messagetarget is not null)"
                + " AND ( IFNULL(IsSchedule, 0) = 0 OR ( IFNULL(IsSchedule, 0) = 1"
                + " AND now() > ScheduleSendDate)) AND to_days(GenerateDate) = to_days(now()) ORDER BY generatedate asc  limit 50";
