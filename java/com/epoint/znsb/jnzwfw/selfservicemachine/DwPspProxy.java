...
public class DwPspProxy {
    ...
    public String invokeService(String licenseKey , String serviceName, String operationName, String xmlPara) {
        ...
        try {
            ...
            //设置请求的超时时间
            RequestConfig rconfig = RequestConfig.custom().setConnectTimeout(200000).setSocketTimeout(200000).build();
            ...
            logger.error("代理调用服务, 地址为：" + dwpspserverurl +", 提示信息：" + e.getMessage());
        }
        finally {
            try {
                if (entity != null) {
    ...
    public static String queryString(String serviceName,String operationName,Map<String,String> map){
       ...
       //String operationName = "queryCardInfoBySfzhm";
        String xmlPara = "";
        if(map.size() > 0){
             xmlPara = "<?xml version=\"1.0\" encoding=\"GBK\"?><p>";

            for (String key : map.keySet()) {
                xmlPara = xmlPara + "<s " + key +"=\""+map.get(key)+"\"/>";
            }
            xmlPara = xmlPara + "</p>";
        }
    ...
    public static String queryYBString(String serviceName,String operationName,Map<String,String> map){
    ...
    public static String readXml(String xmlresult){
        JSONObject obj = new JSONObject();
            try {
                ...
                Iterator iter = rootstore.elementIterator("s"); // 获取根节点下的子节点s
                // 遍历s节点
                while (iter.hasNext()) {
