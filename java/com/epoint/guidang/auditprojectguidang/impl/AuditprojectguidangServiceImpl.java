package com.epoint.guidang.auditprojectguidang.impl;
import java.util.List;

import org.springframework.stereotype.Component;

import com.alibaba.dubbo.config.annotation.Service;
import com.epoint.guidang.auditprojectguidang.api.IAuditprojectguidangService;
import com.epoint.guidang.auditprojectguidang.entity.Auditprojectguidang;
/**
 * 办件归档记录表对应的后台service实现类
 * 
 * @author chengninghua
 * @version [版本号, 2018-01-05 14:04:07]
 */
@Component
@Service
public class AuditprojectguidangServiceImpl implements IAuditprojectguidangService
{
    /**
     * 
     */
    private static final long serialVersionUID = -6515899840121877113L;

    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
	@Override
    public int insert(Auditprojectguidang record) {
        return new AuditprojectguidangService().insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
	@Override
    public int deleteByGuid(String guid) {
        return new AuditprojectguidangService().deleteByGuid(guid);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
	@Override
    public int update(Auditprojectguidang record) {
        return new AuditprojectguidangService().update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
	@Override
    public Auditprojectguidang find(Object primaryKey) {
       return new AuditprojectguidangService().find(primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
	@Override
    public Auditprojectguidang find(String sql, Object... args) {
        return new AuditprojectguidangService().find(args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
	@Override
    public List<Auditprojectguidang> findList(String sql, Object... args) {
       return new AuditprojectguidangService().findList(sql,args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
	@Override
    public List<Auditprojectguidang> findList(String sql, int pageNumber, int pageSize, Object... args) {
       return new AuditprojectguidangService().findList(sql,pageNumber,pageSize,args);
    }

}
