 ...
 */
@RestController("sxauditresultaddaction")
@Scope("request")
public class SXAuditResultAddAction extends BaseController
{
    ...
    */
    ...
    public void pageLoad() {
        ...
        if (StringUtil.isNotBlank(sharematerialguid)) {
           //判断共享材料是否配置多个
            String materialname="";
           String[] materialguids=sharematerialguid.split(",");
            for (int i = 0; i < materialguids.length; i++) {
                CertCatalog certCatalog = certConfigExternalImpl.getCatalogByCatalogid(materialguids[i], areacode);
                if(certCatalog!=null){
                    if(i==materialguids.length-1){
                        materialname+=certCatalog.getCertname();
                    }else{
                        materialname=materialname+certCatalog.getCertname()+";";
                    }
                }
