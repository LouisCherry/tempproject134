package com.epoint.audittask.extension.action;
 ...
 */
@RestController("jnaudittaskextendaddaction")
@Scope("request")
@SuppressWarnings("unchecked")
public class JNAuditTaskExtendAddAction extends BaseController {
     ...
     * 是否发送补正短信
     */
    private List<SelectItem> isSendbzModal;
    /**
     * 自建系统模式代码项
     */
     ...
     * 是否接件即受理
     */
    private List<SelectItem> jiejianandshouliModeModal;
    /**
     * 何时收费代码项
     */
    ...
    public void pageLoad() {
        ...
        if (StringUtil.isNotBlank(taskGuid)) {
            if (StringUtil.isBlank(copyTaskGuid)) {
                ...
                // 窗口事项复制和窗口事项变更的时候初始化从数据库读取
                    ...
                    auditTask = auditTaskServie.getAuditTaskByGuid(copyTaskGuid, false).getResult();
                    auditTaskExtension = auditTaskExtensionImpl.getTaskExtensionByTaskGuid(copyTaskGuid, false).getResult();
                    AuditTaskExtension auditTaskExtensionNow = auditTaskExtensionImpl.getTaskExtensionByTaskGuid(taskGuid, false).getResult();
                    //变更事项的时候初始化是否勘验字段
                    if (auditTaskExtension!=null && auditTaskExtensionNow!=null && auditTaskExtensionNow.get("is_ky") != null) {
                        auditTaskExtension.set("is_ky", auditTaskExtensionNow.get("is_ky"));
                    }
            ...
            // 背景核验
            if (auditTaskExtension != null && StringUtil.isBlank(auditTaskExtension.getStr("is_bgv"))) {
        ...
        // 根据是否工改字段控制手风琴展示
        if (auditTask != null && ("1").equals(auditTask.getStr("is_ggtask"))) {
    ...
    private void init() {
        ...
        if (StringUtil.isBlank(val = getViewData(key))) {
            DeptSelfBuildSystem selfBuildSystem = selfBuildSystemService.findDeptSelfBuildSystemByTaskId(auditTask.getTask_id());
    ...
    public void submit() {
        ...
        if (StringUtil.isNotBlank(auditTaskExtension.getStr("keshinum"))) {
            auditTaskExtension.set("keshiname", iCodeItemsService.getItemTextByCodeName("科室名称", auditTaskExtension.getStr("keshinum")));
        }
...
/*			String RabbitMQMsg = "handleSerachIndex:" + SendType + ";" + taskGuid
    ...
    public List<SelectItem> getIsSendbzModal() {
        if (isSendbzModal == null) {
            isSendbzModal = DataUtil.convertMap2ComboBox(
                    (List<Map<String, String>>) CodeModalFactory.factory("单选按钮组", "是否", null, false));
        }
        return this.isSendbzModal;
    }
    ...
    public List<SelectItem> getJiejianandshouliModeModal() {
        if (jiejianandshouliModeModal == null) {
            jiejianandshouliModeModal = DataUtil.convertMap2ComboBox((List<Map<String, String>>) CodeModalFactory
                    .factory(EpointKeyNames9.CHECK_SELECT_GROUP, "是否", null, false));
        }
        return this.jiejianandshouliModeModal;
    }
