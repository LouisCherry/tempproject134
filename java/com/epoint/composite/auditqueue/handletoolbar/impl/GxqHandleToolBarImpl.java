...
@Component
@Service
public class GxqHandleToolBarImpl implements IGxqHandleToolBar {
    ...
    public AuditCommonResult<String> initQueueLogin(String windowguid, String userguid, String displayname,
        ...
        try {
            ...
            if (QueueConstant.Common_yes_String.equals(AS_IS_USE_LED)) {
                ...
                insertledmessage(ledcontent, window.getWindowno());
            }
            // 窗口屏推送
    ...
    public AuditCommonResult<String> pauseQueue(String windowguid, String windowno, String centerguid) {
        ...
        try {
            ...
            if (StringUtil.isNotBlank(config) && QueueConstant.Common_yes_String.equals(config)) {
                ...
                insertledmessage(ledcontent, windowno);
            }
            // 推送暂停信息
    ...
    public AuditCommonResult<String> reCallQueue(String qno, String windowguid, String windowno, String centerguid) {
        ...
        try {
            ...
            if (!QueueConstant.Current_None.equals(qno)) {
                sendqnoledmessage(centerguid, windowno, qno);
            } else {
                String ledcontent = handleConfigservice.getFrameConfig("AS_QUEUE_LED_FREE", centerguid)
                        .getResult();
                if (StringUtil.isBlank(ledcontent)) {
                    ledcontent = "欢迎光临！";
                }
    ...
    public AuditCommonResult<String> nextQueue(String qno, String windowguid, String windowno, String centerguid,
        ...
        try {
            ...
            String nextqno = handlequeueservice.getNextQNO(windowguid, windowno, centerguid, UserGuid, UseCall).getResult();
            // 取出下一个号
            result.setResult(
                    nextqno
            );

            if (!QueueConstant.Current_None.equals(nextqno)) {
                sendqnoledmessage(centerguid, windowno, nextqno);
            } else {
                String ledcontent = handleConfigservice.getFrameConfig("AS_QUEUE_LED_FREE", centerguid)
                        .getResult();
                if (StringUtil.isBlank(ledcontent)) {
        ...
        try {
            if (StringUtil.isNotBlank(qno) && !QueueConstant.Window_Bar_status_None.equals(qno)) {
                ...
                if (StringUtil.isNotBlank(useBlackList) && QueueConstant.CONSTANT_STR_ONE.equals(useBlackList)) {
                    ...
                    if (StringUtil.isNotBlank(currentPassQueue) && StringUtil.isNotBlank(cardNo)) {
                        // 查询过号违约记录
                        AuditQueueBreachRecord passQueue = breachRecordService.getPassQueueRecord(qno, centerguid).getResult();
                        breachRecordService.insert(passQueue);
                        // 获取系统参数中的过号统计时间和最大次数
                        String passqnoCountTimeStr = configService.getFrameConfigValue("AS_PASSQNO_COUNT_TIME");
                        String passqnoMaxAllowCountStr = configService.getFrameConfigValue("AS_PASSQNO_MAX_ALLOW_COUNT");
                        String autoReleaseBlackTimeStr = configService.getFrameConfigValue("AS_AUTO_RELEASEBLACK_TIME");
                        int passqnoCountTime = StringUtil.isNotBlank(passqnoCountTimeStr) ? Integer.parseInt(passqnoCountTimeStr) : 30;
                        int passqnoMaxAllowCount = StringUtil.isNotBlank(passqnoMaxAllowCountStr) ? Integer.parseInt(passqnoMaxAllowCountStr) : 3;
                        int autoReleaseBlackTime = StringUtil.isNotBlank(autoReleaseBlackTimeStr) ? Integer.parseInt(autoReleaseBlackTimeStr) : 30;
                        // 查询当前身份证过号违约次数
                        Integer passCount = breachRecordService.getPassQNOCountByCard(cardNo, EpointDateUtil.addDay(new Date(), (-1) * passqnoCountTime), new Date()).getResult();
                        ...
                        if (passCount == passqnoMaxAllowCount || passCount > passqnoMaxAllowCount) {
                            ...
                            messageservice.insertSmsMessage(UUID.randomUUID().toString(), messagecontent, new Date(), 0,
                                    new Date(), currentPassQueue.getPhone(), UUID.randomUUID().toString(), "", "", "",
                                    "", "", null, false, "");
                        }
    ...
    public AuditCommonResult<String> nextPassQueue(String qno, String passqno, String windowguid, String windowno,
        ...
        try {
            if (StringUtil.isNotBlank(passqno) && StringUtil.isNotBlank(qno)) {
                ...
                if (!QueueConstant.Current_None.equals(passqno)) {
                    sendqnoledmessage(centerguid, windowno, passqno);
                } else {
                    String ledcontent = handleConfigservice.getFrameConfig("AS_QUEUE_LED_FREE", centerguid)
                            .getResult();
                    if (StringUtil.isBlank(ledcontent)) {
    ...
    public void sendqnoledmessage(String centerguid, String WindowNo, String QNO) {
        IHandleConfig handleConfigservice = ContainerFactory.getContainInfo().getComponent(IHandleConfig.class);
        IHandleConfig configservice = ContainerFactory.getContainInfo().getComponent(IHandleConfig.class);
        String AS_IS_USE_LED = configservice.getFrameConfig("AS_IS_USE_LED", centerguid).getResult();
        if (QueueConstant.CONSTANT_STR_ONE.equals(AS_IS_USE_LED)) {
            String ledcontent = handleConfigservice.getFrameConfig("AS_QUEUE_LED_CALL", centerguid).getResult();
    ...
    public void insertledmessage(String content, String windowno) {
        IWindowLed windowled = ContainerFactory.getContainInfo().getComponent(IWindowLed.class);
        windowled.inset(windowno, content);
    }
