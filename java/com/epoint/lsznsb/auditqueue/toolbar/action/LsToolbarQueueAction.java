package com.epoint.lsznsb.auditqueue.toolbar.action;
...
@RestController("lstoolbarqueueaction")
@Scope("request")
public class LsToolbarQueueAction extends BaseController
{
    ...
    @Autowired
    private LsIHandleToolBar lshandletoolbarservice;
    @Autowired
    ...
    public void pageLoad() {
        String call = configService.getFrameConfig("AS_QUEUE_MESSAGE_CALL", zwfwuserSession.getCenterGuid(), "1")
                .getResult();
        if ("0".equals(call)) {
            issendmsg = false;
        }
    ...
    public void delayclick() {
        if (!QueueConstant.Window_WorkStatus_Pause.equals(getHidStatus())) {
            ...
            addCallbackParam("QNO",
                    lshandletoolbarservice.startQueue(zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
    ...
    public void recallclick(String QNO) {
        lshandletoolbarservice.reCallQueue(QNO, zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
                zwfwuserSession.getCenterGuid());
    ...
    public void nextclick(String QNO) {
        addCallbackParam("QNO",
                lshandletoolbarservice.nextQueue(QNO, zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
                        zwfwuserSession.getCenterGuid(), userSession.getUserGuid(), issendmsg).getResult());
        GetWaitNum();
    }
    ...
    public void AutoRefresh() {
        addCallbackParam("QNO",
                lshandletoolbarservice.getNextQNO(zwfwuserSession.getWindowGuid(), zwfwuserSession.getWindowNo(),
                        zwfwuserSession.getCenterGuid(), userSession.getUserGuid(), issendmsg).getResult());
        GetWaitNum();
    }
