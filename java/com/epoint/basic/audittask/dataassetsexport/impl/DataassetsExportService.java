package com.epoint.basic.audittask.dataassetsexport.impl;
import java.util.List;

import com.epoint.basic.audittask.dataassetsexport.api.entity.DataassetsExport;
import com.epoint.common.util.ZwfwConstant;
import com.epoint.core.dao.CommonDao;
import com.epoint.core.dao.ICommonDao;
import com.epoint.core.grammar.Record;
import com.epoint.core.utils.string.StringUtil;

/**
 * 导出记录对应的后台service
 * 
 * @author 95453
 * @version [版本号, 2020-08-24 17:27:23]
 */
public class DataassetsExportService
{
 /**
     * 数据增删改查组件
     */
    protected ICommonDao baseDao;

    public DataassetsExportService() {
        baseDao = CommonDao.getInstance();
    }
    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(DataassetsExport record) {
        return baseDao.insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public <T extends Record> int deleteByGuid(String guid) {
        T t = baseDao.find(DataassetsExport.class, guid);
        return baseDao.delete(t);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(DataassetsExport record) {
        return baseDao.update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public DataassetsExport find(Object primaryKey) {
        return baseDao.find(DataassetsExport.class, primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public DataassetsExport find(String sql,  Object... args) {
        return baseDao.find(sql, DataassetsExport.class, args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<DataassetsExport> findList(String sql, Object... args) {
        return baseDao.findList(sql, DataassetsExport.class, args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<DataassetsExport> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return baseDao.findList(sql, pageNumber, pageSize, DataassetsExport.class, args);
    }

	/**
     * 查询数量
     * 
     * @param sql
     *            执行语句
     * @param args
     *            参数
     * @return Integer
     */
    public Integer countDataassetsExport(String sql, Object... args){
        return baseDao.queryInt(sql, args);
    }
    
    public int getFlowsnNumByDate(String flowsn) {
        String sql ="select flowsn from DATAASSETS_EXPORT where flowsn like ? order by flowsn desc";
        String str = baseDao.queryString(sql, flowsn +"%");
        if(StringUtil.isBlank(str) || str.length() != 14){
            return ZwfwConstant.CONSTANT_INT_ZERO;
        }
        return Integer.parseInt(str.substring(11));
    }
}
