 ...
 * 
 */
public class AuditOrgaWindowYjsService
{
    /**
     * 通用dao操作
     */
    ...
    public AuditOrgaWindowYjsService() {
     ...
     * @see [类、类#方法、类#成员]
     */
    public void insertWindow(AuditOrgaWindowYjs auditWindow) {
    ...
    public String getOuguidByWindowGuid(String windowguid) {
        Record record = commonDao.find("select ouguid from audit_orga_window where rowguid=?", Record.class,
                windowguid);
        String ouguid = "";
        if (record != null) {
    ...
    public List<Record> getoulistBycenterguid(String centerGuid) {
        List<Record> record = commonDao.findList("select distinct(ouguid) from audit_orga_window where centerguid = ?",
                Record.class, centerGuid);
        return record;
    }
    ...
    public List<AuditOrgaWindow> getAllByCenter(String centerguid) {
        List<AuditOrgaWindow> list = new ArrayList<AuditOrgaWindow>();
        list = commonDao.findList("select * from audit_orga_window where  centerguid=? ", AuditOrgaWindow.class,
                centerguid);
        return list;
    }
    ...
    public List<AuditOrgaWindow> getWindowListByUserGuidAndCondition(String userGuid,
            Map<String, String> conditionMap) {
        StringBuffer sb = new StringBuffer();
        SQLManageUtil sUtil = new SQLManageUtil();
        sb.append(
                "select audit_orga_window.* from audit_orga_window,audit_orga_windowuser where audit_orga_window.RowGuid=audit_orga_windowuser.WINDOWGUID and userguid=?1");
    ...
    public AuditOrgaWindow getFirstWindowByCenterGuid(String centerguid) {
        String sql = "select * from audit_orga_window where centerguid = ? order by ordernum desc limit 0,1";
        return commonDao.find(sql, AuditOrgaWindow.class, centerguid);
    }
    
    public List<AuditSpBusiness> getBusinessByAreacode(String areacode,String businessname) {
        String sql = "";
        if ("370800".equals(areacode)) {
        	sql = "select * from audit_sp_business where businessname like ?";
        	return commonDao.findList(sql, AuditSpBusiness.class, "%"+businessname+"%");
        }else {
        	sql = "select * from audit_sp_business where areacode = ? and businessname like ?";
        	return commonDao.findList(sql, AuditSpBusiness.class, areacode, "%"+businessname+"%");
        }
    }
    ...
    public List<AuditOrgaArea> getAreacodeList(String areacode) {
    	String sql = "";
        if ("370800".equals(areacode)) {
        	sql = "select distinct b.XiaQuCode,b.xiaquname from audit_sp_business a join audit_orga_area b on a.areacode = b.XiaQuCode where a.del = '0' and a.businesstype = '2'";
        	return commonDao.findList(sql, AuditOrgaArea.class);
        }else {
        	sql = "select distinct b.XiaQuCode,b.xiaquname from audit_sp_business a join audit_orga_area b on a.areacode = b.XiaQuCode where a.areacode = ? and a.del = '0' and a.businesstype = '2'";
        	return commonDao.findList(sql, AuditOrgaArea.class, areacode);
        }
    }
    
    public List<AuditSpBusiness> getBusinessDetailByAreacode(String areacode) {
        String sql = "select * from audit_sp_business where areacode = ? and del = '0' and businesstype = '2'";
        return commonDao.findList(sql, AuditSpBusiness.class, areacode);
    }
    
    public List<AuditSpBusiness> getBusinessDetailByRowguid(String rowguid) {
        String sql = "select * from audit_sp_business where rowguid = ?";
        return commonDao.findList(sql, AuditSpBusiness.class, rowguid);
    }
    
    public List<AuditOrgaWindowYjs> getYjsListByRowguid(String rowguid,String windowguid) {
        String sql = "select * from audit_orga_windowyjs where taskguid = ? and windowguid = ?";
        return commonDao.findList(sql, AuditOrgaWindowYjs.class, rowguid,windowguid);
    }
    
    
    public List<AuditOrgaWindowYjs> getAllYjs(String windowguid) {
        String sql = "select * from audit_orga_windowyjs where windowguid = ? order by ordernum desc ";
        return commonDao.findList(sql, AuditOrgaWindowYjs.class, windowguid);
    }
    
    
    public <T extends Record> int deleteByGuid(String guid) {
        T t = commonDao.find(AuditOrgaWindowYjs.class, guid);
        return commonDao.delete(t);
    }
    
    
    public void deleteWindowTaskByTaskGuid(String taskguid) {
    	String sql = "delete from audit_orga_windowyjs where taskguid=?";
