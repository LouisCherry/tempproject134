 ...
 */
@RestController("certinfolistzzlbaction")
@Scope("request")
public class CertInfoListZzlbAction extends BaseController
{
    private static final long serialVersionUID = 1L;

    transient static Logger logger = LogUtil.getLog(CertInfoListZzlbAction.class);
     ...
     * 日志
     */
    private Logger log = LogUtil.getLog(CertInfoListZzlbAction.class);
   ...
   /* public LazyTreeModal9 getTreeModel() {
            ...
            treeModel2 = new TreeModel()
            {
                ...
                public List<TreeNode> fetch(TreeNode treeNode) {
                    ...
                    if (treeData == null) {
                        ...
                        List<CertCatalog> certCataloglist = baseinfoService.findCertCatalogZzlbByOuguid(ouguid);
                          if(StringUtil.isNotBlank(certCataloglist)) {
                              for (CertCatalog certCatalog : certCataloglist) {
                                  TreeNode pNode = new TreeNode();
                                  Map<String,Object> map = new HashMap<String,Object>();
                                  pNode.setText(certCatalog.getCertname());
                                  pNode.setId(certCatalog.getCertcatalogid());
                                  pNode.setPid(root.getId());
                                  pNode.setLeaf(true);
                                  map.put("objectcode","1");
                                  pNode.setColumns(map);
                                  pNode.setExpanded(false);// 展开下一层节点
            ...
            model = new DataGridModel<CertInfo>()
            {
                ...
                public List<CertInfo> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if (CertConstant.CONSTANT_STR_ZERO.equals(getIspid())) {
                        ...
                        if (StringUtil.isNotBlank(certcatalogid)) {
                            ...
                            if (catalog != null) {
                                ...
                                yyyzCertCode = catalog.getStr("yyyzCertCode");
                            }
                            else {
                                this.setRowCount(0);
                                return certinfoList;
                    ...
                    if (StringUtil.isNotBlank(certcatalogid) || StringUtil.isNotBlank(dataBean.getCertno())
                        ...
                        if (StringUtil.isBlank(yyyzCertCode)) {
                            ...
                            if (ValidateUtil.isNotBlankCollection(certinfoList)) {
                                for (CertInfo certinfo : certinfoList) {
                                    certinfo.put("yyyzCertCode", yyyzCertCode);
                                    certinfo.setCertownername(FormatUtil
                                            .getdecryptSM4ToData(certinfo.getCertownername(), certcatalogid, 0));
                                }
                            }
    ...
    public Map<String, String> generateCertDoc(String cliengGuid, boolean isPrintCopy, CertInfo certInfo) {
        ...
        try {
            ...
            if (StringUtil.isNotBlank(certClass) && ReflectUtil.exist(certClass)) {
                ...
                returnMap = generateBizlogic.generatePrintDocSupportCopy(cliengGuid, isPrintCopy,
     ...
     */
    public void licenseBack(String guid) {
        try {

            logger.info("=====certguid=====:" + guid);
            IConfigService configService = ContainerFactory.getContainInfo().getComponent(IConfigService.class);
            String serialNo = null;
            BusinessLicenseBaseinfo baseinfo = baseinfoService.getBaseinfoByCertguid(guid);
            if (baseinfo != null) {
                serialNo = baseinfo.getSerialNo();
            }
            else {
                serialNo = guid.replace("-", "");
            }
            logger.info("=====serialNo=====:" + serialNo);
            String url = configService.getFrameConfigValue("AS_YYYZ_LicenseBackUrl");
            JSONObject jsonObject = new JSONObject();
            JSONObject params = new JSONObject();
            params.put("type", "1");
            params.put("biguid", serialNo);
            params.put("certrowguid", guid);
            jsonObject.put("token", "Epoint_WebSerivce_**##0601");
            jsonObject.put("params", params);
            String returnstr = HttpUtil.sendpost(url, jsonObject);

            //{"massage":"success","status":"1"}
            logger.info("=====returnstr=====:" + returnstr);
            if (StringUtil.isNotBlank(returnstr)) {
                JSONObject retJson = JSONObject.parseObject(returnstr);
                String massage = retJson.getString("massage");
                String status = retJson.getString("status");
                if ("success".equals(massage) && "1".equals(status)) {
                    addCallbackParam("msg", "证照信息回传成功！");
                }
                else {
                    addCallbackParam("msg", "证照信息回传失败！具体原因:" + massage);
                ...
                addCallbackParam("msg", "证照信息回传失败！具体原因:" + returnstr);
            }
        }
        catch (Exception e) {
            addCallbackParam("msg", "证照信息回传失败！具体原因:" + e);
        }
    }
