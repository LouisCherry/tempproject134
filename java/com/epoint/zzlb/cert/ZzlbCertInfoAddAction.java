 ...
 */
@RestController("zzlbcertaddaction")
@Scope("request")
public class ZzlbCertInfoAddAction extends BaseController
{
    ...
    @Autowired
    private IAuditSpIntegratedCompany auditSpIntegratedCompany;
     ...
     * 业务实例
     */
    private AuditSpIntegratedCompany spIntegratedCompany;
    ...
    public void pageLoad() {
        ...
        zzjgdmz = getRequestParameter("zzjgdmz");
        ...
        if (StringUtil.isNotBlank(getRequestParameter("biguid"))) {
        ...
        //获取行业类别和许可项目的文本值；
        String yewuguid = spInstance.getYewuguid();
        Record integratedCompany = businessLicenseExtension.getIntegratedCompanyInfoByRowGuid(yewuguid);
        String industryName="";
        List<CodeItems> industryList = codeItemService.listCodeItemsByCodeName(configService.getFrameConfigValue("industryName"));
        for (CodeItems industryItems : industryList) {
            if(integratedCompany.getStr("industrycategory").equals(industryItems.getItemValue())) {
                industryName=industryItems.getItemText();
            }
        }
        String liscensedName="";
        String[] liscenseds = integratedCompany.getStr("licenseditems").split(",");
        List<CodeItems> licensedList = codeItemService.listCodeItemsByCodeName(industryName);
        List<String> liscensedItemsList = new ArrayList<String>();
        for (CodeItems licensedItems : licensedList) {
            for(String liscensed : liscenseds) {
                if(liscensed.equals(licensedItems.getItemValue())) {
                    liscensedItemsList.add(licensedItems.getItemText());
                }
            }
        }
        liscensedName = StringUtil.join(liscensedItemsList, "、");
        if(StringUtil.isBlank(certownertype)) {
        	certownertype = ZwfwConstant.CERTOWNERTYPE_FR;
        }
        if (StringUtil.isNotBlank(certrowguid)) {
            ...
            if (StringUtil.isNotBlank(certcatalogid)) {
                ...
                if (!isPostback()) {
                    ...
                    String certownername = spInstance.getApplyername();
...
//                        addCallbackParam("certownerno", certownerno);
                    ...
                    //设置需要回显的字段行业类别和许可项目渲染到对应的字段里
                    if(dataBean == null) {
                        dataBean= new CertInfoExtension();
                    }
                    //新增页面，行业类别和许可项目回显到
                    for(CertMetadata metadata : metadataList) {
                        if("行业类别".equals(metadata.getFieldchinesename())) {
                            dataBean.set(metadata.getFieldname(), industryName);
                        }
                        if("许可项目".equals(metadata.getFieldchinesename())){
                            dataBean.set(metadata.getFieldname(), liscensedName);
                        }
                    }
                    // 返回渲染的字段(子表新增模式)
                    addCallbackParam("controls", this.generateMapUseSubExtension("certaddaction", metadataList,
                    ...
                    if("002".equals(certownertype)) {
                        List<CodeItems> certTypeList = codeItemService.listCodeItemsByCodeName("申请人用来唯一标识的证照类型");
                        for (CodeItems certType : certTypeList) {
                            if("统一社会信用代码".equals(certType.getItemText())) {
                                addCallbackParam("certownercerttype", certType.getItemValue());
                                addCallbackParam("certownerno", zzjgdmz);
                            }
                        }
                    }
        ...
        if (!isPostback()) {
            if (CertConstant.CONSTANT_INT_ONE.equals(certCatalog.getIsmultiowners())) {
                ...
                if (StringUtil.isNotBlank(no)) {
                    ...
                    JSONObject jsonObject = new JSONObject();
                    String ownername = spInstance.getApplyername();
...
//                        jsonObject.put("ownertype", ownercerttype);
        ...
        addCallbackParam("repeatFields", repeatFields);
    }
    ...
    public void add(String params, String cliengguids) {
        ...
        if ("保存成功！".equals(msg)) {
        	spInstance.set("certrowguid", certinfo.getRowguid());
            addViewData("rowguid", certinfo.getRowguid());
            auditSpInstanceService.updateAuditSpInstance(spInstance);
        }
        addCallbackParam("msg", msg);
    }

    public String setCertValue(String params, String cliengguids, JSONObject cliengguidsObject) {
            ...
            for (CertMetadata metadata : metadataList) {
                if ("image".equals(metadata.getFieldtype().toLowerCase())
                    ...
                    if(StringUtil.isBlank(cliengguid)){
                        cliengguid = getViewData("piccliengguid");
                    }
                    if (StringUtil.isNotBlank(cliengguid)) {
                        int attchCount = attachService.getAttachCountByClientGuid(cliengguid);
                              //  .getAttachList(certCatalog.getCopytempletcliengguid()).size();
                        if (attchCount == 0) {
    ...
    public void generateCert(String parentguidList, String cliengguids, String isGenerateCopy) throws ParseException {
        ...
        // 生成PDF
        msg = setCertValue(parentguidList, cliengguids, jsonObject);
        if(StringUtil.isNotBlank(msg)){
            addCallbackParam("msg", msg);
            return;
        }
    ...
    public void printCert(String isPrintCopy) {
        ...
        if (certCatalog != null) {
            ...
            if (catalog == null) {
                iAuditCertCatalog.createAuditCatalogByCertCatalog(certCatalog.getCertcatalogid(),
                        certCatalog.getTempletcliengguid(), certCatalog.getCopytempletcliengguid(),
                ...
                if (attachList != null && attachList.size() > 0) {
                    for (JSONObject json : attachList) {
                        AttachUtil.saveFileInputStream(UUID.randomUUID().toString(), certCatalog.getTempletcliengguid(),
                    ...
                    if (copyAttachList != null && copyAttachList.size() > 0) {
                        for (JSONObject json : copyAttachList) {
                            AttachUtil.saveFileInputStream(UUID.randomUUID().toString(),
                                    certCatalog.getCopytempletcliengguid(), json.getString("attachname"),
                ...
                if (catalog.getVersion() != null && !catalog.getVersion().equals(certCatalog.getVersion())) {
                    iAuditCertCatalog.updateAuditCertCatalog(catalog, certCatalog.getTempletcliengguid(),
                            certCatalog.getCopytempletcliengguid(), certCatalog.getVersion());
                    if (StringUtil.isBlank(certCatalog.getTempletcliengguid())) {
                        addCallbackParam("msg", "该证照模板不存在！");
                        return;
                    }
                    //将模板附件保存至本地附件库
                    /*List<JSONObject> attachList = iCertAttachExternal.getAttachList(certCatalog.getTempletcliengguid(),
                    ...
                    if (attachList != null && attachList.size() > 0) {
                        for (JSONObject json : attachList) {
                            AttachUtil.saveFileInputStream(UUID.randomUUID().toString(),
                                    certCatalog.getTempletcliengguid(), json.getString("attachname"),
                                    json.getString("contentype"), "证照检索新增", json.getLong("size"),
                                    iCertAttachExternal.getAttach(json.getString("attachguid"), areacode),
                                    userSession.getUserGuid(), userSession.getDisplayName());
                        }
        ...
        String sql = "select * from cert_catalog where ISHISTORY=0 and CERTCATALOGID=?1";
        AuditCertCatalog auditCertCatalog = CommonDao.getInstance().find(sql, AuditCertCatalog.class, certCatalog.getCertcatalogid());
...
//                .getAuditCertCatalogByCatalogid(certCatalog.getCertcatalogid()).getResult();
        ...
        if (ZwfwConstant.CONSTANT_STR_ONE.equals(isPrintCopy)) { // 打印副本
            ...
            // 没打印，打印
            returnMap = generateCertDoc(auditCertCatalog.getStr("TdTempletcliengguid"), false);
        }
...
//                auditSpInstanceService.updateAuditSpInstance(spInstance);
    ...
    public static void insertImagetoBookmark(Document doc, Map<String, Record> imageMap, boolean isUseQrcode,
        ...
        try {
            ...
            if (isUseQrcode && StringUtil.isNotBlank(qrcodeText)) {
                ...
                int qcodeWidth = GenerateUtil.parseInteger(configService.getFrameConfigValue("yyyz_qrcode_width"), 100);
                int qcodeHeight = GenerateUtil.parseInteger(configService.getFrameConfigValue("yyyz_qrcode_height"), 100);
   ...
   public void markFaFang(String cliengguid) {
    ...
    public Map<String, Object> generateBasicDefaultMap(List<CertMetadata> certMetadataList, boolean isdetail,
        ...
        for (CodeItems codeItem : codeItems) {
            ...
            if ("certownercerttype".equals(codeItem.getItemValue())) {
                ...
                recordMap.put("onvaluechanged", "certownercerttypechanged");
            }
    ...
    public FileUploadModel9 getFileUploadModel(String fieldName, String cliengguid) {
        addViewData("piccliengguid", cliengguid);
    ...
    public FileUploadModel9 getCertFileUploadModel() {
        if (certinfo != null && certFileUploadModel == null && StringUtil.isNotBlank(certinfo.getCertcliengguid())) {
    ...
    public String getLicenseditems() {
        return licenseditems;
    }

    public void setLicenseditems(String licenseditems) {
        this.licenseditems = licenseditems;
    }
}
