package com.epoint.zzlb.auditsphandle;
...
@RestController("zzlbhandlebusinesslistaction")
@Scope("request")
public class ZzlbHandleBusinessListAction extends BaseController
{
    /**
     * 
     */
            ...
            model = new DataGridModel<AuditSpBusiness>()
            {
                ...
                public List<AuditSpBusiness> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    if(pageData!=null){
                        ...
                        this.setRowCount(0); 
                        return new ArrayList<AuditSpBusiness>();
                    }
                }
            };
        }
        ...
        if(ZwfwConstant.CONSTANT_STR_ONE.equals(auditSpBusiness.getDel())){
            ...
            if (StringUtil.isNotBlank(filepath)) {
                if ("1".equals(businessType)) {
                    // 建设项目
                    registerUrl = filepath + "?businessGuid=" + businessGuid;
                }
                else if ("2".equals(businessType)||"3".equals(businessType)) {
                    // 一般并联审批
                    registerUrl = filepath + (filepath.substring(filepath.lastIndexOf("/"), filepath.length()) + "add")
                            +"?businessGuid="+ businessGuid;
                }
            }else {
                if ("1".equals(businessType)) {
                    // 建设项目
                    registerUrl = "epointzwfw/auditsp/auditsphandle/handleitemregister?businessGuid=" + businessGuid;
                }
                else if ("2".equals(businessType)||"3".equals(businessType)) {
                    // 一般并联审批
                    registerUrl = "epointzwfw/auditsp/auditspintegrated/auditspintegratedadd?businessGuid=" + businessGuid;
                }
            }
