...
@RestController("jnwebuploaderdaorutwoaction")
@Scope("request")
public class JNWebUploaderDaorutwoaction extends BaseController {
    ...
    private Logger logger = Logger.getLogger(JNWebUploaderDaorutwoaction.class);
            ...
            dataImportModel = new DataImportModel9(new ImportExcelHandler() {
                ...
                public String saveExcelData(String filename, String sheetName, int sheet, int curRow, int totalRows,
                    ...
                    try {
                        ...
                        lcprojecttwo project = new lcprojecttwo();
                        ...
                        if (sheet == 0) {
                            ...
                            if (curRow > 1) {
                                ...
                                for (int i = 0; i < data.length; i++) {
                                    ...
                                    switch (key) {
                                        ...
                                        case "联系人":
                                            ...
                                            break;
                                        case "联系电话":
                                            boolean isValid = isValidChineseMobileNumber(value);
                                            if (isValid) {
                                                project.setContactmobile(value);
                                                break;
                                            } else {
                                                return "导入失败！请检查手机号格式！";
                                            }
                                        case "承诺时间":
                                ...
                                if (project != null) {
                                    ...
                                    if (task != null){
                                        if (StringUtil.isNotBlank(task.getStr("inner_code"))) {
                                            baseinfo.setItemno(task.getStr("inner_code"));
                                        } else {
                                            baseinfo.setItemno("无");
                                        }
                                        String materialname = "";
                                        if (taskToMaterail.containsKey(task.getItem_id())) {
                                            materialname = taskToMaterail.get(task.getItem_id());
                                        } else {
                                            ...
                                            taskToMaterail.put(task.getItem_id(), materialname);
                                        }
    ...
    public boolean isValidChineseMobileNumber(String number) {
        ...
        if (model == null) {
            model = new DataGridModel<Record>() {
                ...
                public List<Record> fetchData(int first, int pagesize, String arg2, String arg3) {
                    ...
                    log.info("ouguid:"+ouguid+"areacode:"+areacode);
                    List<Record> lst = service.finListTwo(first, pagesize, ouguid, areacode, banjiedateStart,
                            banjiedateEnd, certnum);
                    Integer row = service.finTotalTwo(ouguid, areacode, banjiedateStart, banjiedateEnd, certnum);
