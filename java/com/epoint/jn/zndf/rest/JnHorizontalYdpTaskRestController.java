 ...
 */
@RestController
@RequestMapping("/jnhorizontalydptask")
public class JnHorizontalYdpTaskRestController
{
	@Autowired
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/getCategoryPage", method = RequestMethod.POST,produces = {"application/json;charset=UTF-8"})
    public String getCategoryPage(@RequestBody String params) {
        try {
            JSONObject json = JSON.parseObject(params);
            JsonUtils.checkUserAuth(json.getString("token"));//token验证
            JSONObject obj = (JSONObject) json.get("params");
            String centerguid = obj.getString("centerguid");
            String userType = obj.getString("UserType");//用户类型  企业: company ；个人:  person
            String targetId = obj.getString("targetId");//查询类别 主题:theme 企业: industry 部门: department
            String page = obj.getString("page");
            String pagesize = obj.getString("pagesize");
            int startresult=Integer.parseInt(page)*Integer.parseInt(pagesize);
            int maxresult=(Integer.parseInt(page)+1)*Integer.parseInt(pagesize);
            ...
            if ("department".equals(targetId)) {
                ...
                if(ourecord!=null&&ourecord.size()>0&&ourecord.size()>startresult){
                    if(maxresult>ourecord.size()) maxresult=ourecord.size();
                    for(int i=startresult;i<maxresult;i++){
                        String record=ourecord.get(i);
                ...
                dataJson.put("totalpage", ourecord.size()/Integer.parseInt(pagesize));
                ...
                if("company".equals(userType)){
                    ...
                    if(dictlist!=null&&dictlist.size()>0&&dictlist.size()>startresult){
                        if(maxresult>dictlist.size()) maxresult=dictlist.size();
                        for (int i = startresult; i < maxresult; i++) {                          
                            String name=dictlist.get(i).getClassname();
                            if(StringUtil.isNotBlank(name)&&name.length()>5){
                        	...
                        	datarecord.put("no", dictlist.get(i).getNo());
                            datalist.add(datarecord);
                        }
                    }
                    dataJson.put("content", datalist);
                    dataJson.put("tabVuew", "ws");
                    dataJson.put("totalpage", dictlist.size()/Integer.parseInt(pagesize));
	...
	public String getTaskList(@RequestBody String params) {
		try {
			...
			dataJson.put("totalpages", String.valueOf(taskpagedata.getRowCount()/10));
			return JsonUtils.zwdtRestReturn("1", "", dataJson);
		} catch (Exception e) {
			return JsonUtils.zwdtRestReturn("0", "出现异常：" + e.getMessage(), "");
		}
	...
	public String getTaskDetailByguid(@RequestBody String params) {
		try {
			...
			JsonUtils.checkUserAuth(json.getString("token"));// token验证
			JSONObject obj = (JSONObject) json.get("params");
			String znsbtaskguid = obj.getString("taskguid");// audittask的唯一键
			JSONObject dataJson = new JSONObject();

			AuditZnsbCentertask centertask = centertaskservice.getDetailbyTaskGuid(znsbtaskguid).getResult();

			String taskguid = "";

			if(centertask != null){

				taskguid =centertask.getTaskguid();
			}
			else{
				return JsonUtils.zwdtRestReturn("0", "不存在该数据！", "");
			}
				
			AuditTask task = taskservice.getAuditTaskByGuid(taskguid, true).getResult();
			String handlewindow=iJnzndf.getWindowByTaskguid(taskguid).getResult();
			// 能办理的窗口编号
			dataJson.put("handlewindow", handlewindow);
		    // 服务指南二维码生成
			String fwurl="http://112.6.110.176:25001/jnzwdt/jnzwdt/pages/jntask/detail.html?taskguid="+taskguid;
			dataJson.put("fwurl", fwurl);
			if (task != null) {
