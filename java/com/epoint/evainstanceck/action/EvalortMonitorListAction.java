 ...
 */
@RestController("evalortmonitorlistaction")
@Scope("request")
public class EvalortMonitorListAction extends BaseController {
    @Autowired
        ...
        if (model == null) {
            model = new DataGridModel<Record>() {
                ...
                public List<Record> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    String strsql = " SELECT e.rowguid as rowguid,e.iszg as iszg,ouname,RECEIVEUSERNAME,ACCEPTUSERGUID,WINDOWNAME,PROJECTNAME,FLOWSN,Evaluatedate,(case when satisfied = '0' THEN 2 else satisfied END) satisfied,IFNULL(e.status,0) as status"
                            + " from audit_online_evaluat e INNER JOIN audit_project p"
                            + " on p.RowGuid = e.ClientIdentifier where clienttype in (10,20)"
                            + " and ACCEPTUSERGUID is NOT NULL and satisfied in('0','1','2') and CENTERGUID='" + centerguid + "'";
