 ...
 */
@RestController("evalortoulistaction")
@Scope("request")
public class EvalortOuListAction extends BaseController {
    @Autowired
        ...
        if (model == null) {
            model = new DataGridModel<Record>() {
                ...
                public List<Record> fetchData(int first, int pageSize, String sortField, String sortOrder) {
                    ...
                    String strsql = " SELECT e.rowguid as rowguid,e.iszg as iszg,ouname,RECEIVEUSERNAME,ACCEPTUSERGUID,WINDOWNAME,PROJECTNAME,FLOWSN,evaluatecontent,clienttype,Evaluatedate,(case when satisfied = '0' THEN 2 else satisfied END) satisfied,IFNULL(e.status,0) as status"
                    ...
                    String strsql1 = " SELECT count(1) as total"
                            ...
                            + " on p.RowGuid = e.ClientIdentifier where clienttype in (10,20)"
                            + " and ACCEPTUSERGUID is NOT NULL and CENTERGUID='" + centerguid + "'";
                    if(StringUtils.isNotBlank(getRequestParameter("ouguid"))){
                        strsql+=" and p.ouguid='"+getRequestParameter("ouguid")+ "'";
                        strsql1+=" and p.ouguid='"+getRequestParameter("ouguid")+ "'";
                    }
                    if(StringUtils.isNotBlank(getRequestParameter("startdate"))){
                        strsql += " and ACCEPTUSERDATE>=str_to_date('" + getRequestParameter("startdate") + "', '%Y-%m-%d %H:%i:%s')";
                        strsql1 += " and ACCEPTUSERDATE>=str_to_date('" + getRequestParameter("startdate") + "', '%Y-%m-%d %H:%i:%s')";
                    }
                    if(StringUtils.isNotBlank(getRequestParameter("enddate"))){
                        strsql += " and ACCEPTUSERDATE<=str_to_date('" + getRequestParameter("enddate") + "', '%Y-%m-%d %H:%i:%s')";
                        strsql1 += " and ACCEPTUSERDATE<=str_to_date('" + getRequestParameter("enddate") + "', '%Y-%m-%d %H:%i:%s')";
                    }
                    if(StringUtils.isNotBlank(getRequestParameter("evalevel"))){
                        strsql+=" and satisfied='"+getRequestParameter("evalevel")+ "'";
                        strsql1+=" and satisfied='"+getRequestParameter("evalevel")+ "'";
                    }
