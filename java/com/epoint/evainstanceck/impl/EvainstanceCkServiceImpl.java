package com.epoint.evainstanceck.impl;

import java.util.Date;
import java.util.List;

import org.springframework.stereotype.Component;

import com.alibaba.dubbo.config.annotation.Service;
import com.epoint.core.grammar.Record;
import com.epoint.evainstanceck.api.IEvainstanceCkService;
import com.epoint.evainstanceck.api.entity.EvainstanceCk;

/**
 * 好差评信息表对应的后台service实现类
 * 
 * @author 31220
 * @version [版本号, 2023-11-06 11:18:19]
 */
@Component
@Service
public class EvainstanceCkServiceImpl implements IEvainstanceCkService
{
    /**
     * 插入数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int insert(EvainstanceCk record) {
        return new EvainstanceCkService().insert(record);
    }

    /**
     * 删除数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int deleteByGuid(String guid) {
        return new EvainstanceCkService().deleteByGuid(guid);
    }

    /**
     * 更新数据
     * 
     * @param record
     *            BaseEntity或Record对象 <必须继承Record>
     * @return int
     */
    public int update(EvainstanceCk record) {
        return new EvainstanceCkService().update(record);
    }

    /**
     * 根据ID查找单个实体
     * 
     * @param clazz
     *            类<必须继承BaseEntity>
     * @param primaryKey
     *            主键
     * @return T extends BaseEntity
     */
    public EvainstanceCk find(Object primaryKey) {
        return new EvainstanceCkService().find(primaryKey);
    }

    /**
     * 查找单条记录
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class，返回一个数组
     *            ;String.class;Integer.class;Long.class]
     * @param args
     *            参数值数组
     * @return T {String、Integer、Long、Record、FrameOu、Object[]等}
     */
    public EvainstanceCk find(String sql, Object... args) {
        return new EvainstanceCkService().find(sql, args);
    }

    /**
     * 查找一个list
     * 
     * @param sql
     *            查询语句
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<EvainstanceCk> findList(String sql, Object... args) {
        return new EvainstanceCkService().findList(sql, args);
    }

    /**
     * 分页查找一个list
     * 
     * @param sql
     *            查询语句
     * @param pageNumber
     *            记录行的偏移量
     * @param pageSize
     *            记录行的最大数目
     * @param clazz
     *            可以是[Record.class(弱类型);FrameOu.class(强类型);Object[].class]
     * @param args
     *            参数值数组
     * @return T extends BaseEntity
     */
    public List<EvainstanceCk> findList(String sql, int pageNumber, int pageSize, Object... args) {
        return new EvainstanceCkService().findList(sql, pageNumber, pageSize, args);
    }

    /**
     * 查询数量
     * 
     * @param sql
     *            执行语句
     * @param args
     *            参数
     * @return Integer
     */
    @Override
    public Integer countEvainstanceCk(String sql, Object... args) {
        return new EvainstanceCkService().countEvainstanceCk(sql, args);
    }

    @Override
    public List<Record> findListByouguidTemp(String leftTreeNodeGuid, Date createdate, String evalevel, String iszg,
            String RECEIVEUSERNAM, int first, int pageSize) {
        // TODO Auto-generated method stub
        return new EvainstanceCkService().findListByouguidTemp(leftTreeNodeGuid, createdate, evalevel, iszg, RECEIVEUSERNAM,
                first, pageSize);
    }
    @Override
    public Record findByflowsn(String flowsn) {
        return new EvainstanceCkService().findByflowsn(flowsn);
    }

    @Override
    public int countListByouguidTemp(String leftTreeNodeGuid, Date createdate, String evalevel, String iszg,
            String RECEIVEUSERNAM) {
        // TODO Auto-generated method stub
        return new EvainstanceCkService().countListByouguidTemp(leftTreeNodeGuid, createdate, evalevel, iszg,
                RECEIVEUSERNAM);
    }

}
